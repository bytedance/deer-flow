    ...
    **kwargs
    ...
    for i, message in enumerate(messages):
        ...
        if isinstance(message, AIMessage) and message.tool_calls:
            oai_msg["tool_calls"] = _convert_to_openai_tool_calls(message.tool_calls, **kwargs)
            ...
            for j, block in enumerate(message.content):
                ...
                # OpenAI audio format
                        ...
                        oai_msg["tool_calls"].append(
                            {
                                ...
                                "function": {
                                    "name": block["name"],
                                    "arguments": json.dumps(block["input"], **kwargs),
                                },
                            }
                        )
                    ...
                    tool_messages.extend(
                        convert_to_openai_messages(
                            [tool_message], text_format=text_format, **kwargs
                        )
                    )
...
def _convert_to_openai_tool_calls(tool_calls: list[ToolCall], **kwargs) -> list[dict]:
    return [
        {
            "type": "function",
            "id": tool_call["id"],
            "function": {
                "name": tool_call["name"],
                "arguments": json.dumps(tool_call["args"], **kwargs),
            },
        }
        for tool_call in tool_calls
    ]
