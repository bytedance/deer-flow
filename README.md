...
>
> The Gateway runs API also supports per-thread FIFO queueing with `multitask_strategy="enqueue"`: additional runs stay `pending` until earlier runs on the same thread finish.
