...
The `task` tool can optionally bind a delegation to a persistent CodingTask through `coding_task_id`. Bound tasks are claimed in the current user/thread TaskGraph before the sub-agent starts and are marked completed only after a successful terminal result. In trusted local mode, `create_coding_worktree(repository_path=...)` accepts the absolute path of the repository selected for that run, including repositories on different local drives, creates the linked Worktree inside that target repository, and persists its full host path on each bound CodingTask. Delegation copies the parent thread data and redirects only the sub-agent workspace to that Worktree, leaving both the parent workspace and the target repository's main checkout unchanged. Host-repository Worktrees currently require `LocalSandboxProvider`; unsupported providers fail explicitly. Failed, cancelled, timed-out, missing, interrupted, or Worktree-preparation executions persist a failure reason. `recover_coding_task` resets a failed task to pending only when the current message history contains a matching structured `ask_clarification` retry approval for that exact task; a model-supplied approval flag is not accepted. Ordinary `task` calls remain unchanged.

This is how DeerFlow handles tasks that take minutes to hours: a research task might fan out into a dozen sub-agents, each exploring a different angle, then converge into a single report — or a website — or a slide deck with generated visuals. One harness, many hands.

The bundled `multi-agent-coding` skill first pauses for explicit plan approval, then persists the stable `coding-analysis -> coding-implementation -> coding-review` DAG with `submit_task_plan`, creates their shared Worktree from the user-selected repository, and passes each stage ID back through `task(coding_task_id=...)` so real delegation results drive the stored workflow. A failed stage stops for a fresh human retry decision before each recovery; the skill never starts an autonomous retry loop.
