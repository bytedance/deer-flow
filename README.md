...
Files presented through `present_files` remain part of the thread's artifact state, and the Web UI restores the artifact panel and selected document after a page refresh. During an active agent run, or while its matching file-write tool is unresolved, the currently selected formal artifact is refreshed automatically so edits become visible without a manual reload.
