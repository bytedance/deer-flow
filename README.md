...
**Strict Tool-Call Recovery**: When a provider or middleware interrupts a tool-call loop, DeerFlow now strips provider-level raw tool-call metadata on forced-stop assistant messages and injects placeholder tool results for dangling calls before the next model invocation. This keeps OpenAI-compatible reasoning models that strictly validate `tool_call_id` sequences from failing with malformed history errors. If a provider reuses a tool-call ID on a later turn, recovery keeps that turn's result attached to its own call, so an interrupted earlier call cannot acquire evidence from the later action.
