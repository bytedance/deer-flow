...
DeerFlow uses `Forwarded` / `X-Forwarded-*` headers to recover the browser-facing scheme and origin only when the TCP peer is trusted by `AUTH_TRUSTED_PROXIES`. Gateway launchers disable Uvicorn's proxy-header middleware so this is the single trust boundary and the check always sees the raw transport peer. The variable is unset by default, which ignores forwarded auth headers. Set it to a comma-separated list of the proxy's IPs or CIDRs, narrowed to your controlled proxy network. For the dev Docker Compose network, use `192.168.200.0/24`; for the production Compose network, determine the subnet assigned to `deer-flow` and set that value explicitly. This is a required migration for HTTPS deployments upgrading from a version that trusted forwarded headers unconditionally. The bundled nginx sets `X-Forwarded-Proto`, but preserves an upstream HTTPS value and does not overwrite every forwarded header. Configure the outer trusted proxy to replace or strip client-supplied forwarding headers before traffic reaches DeerFlow.
