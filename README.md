...
The `task` tool can optionally bind a delegation to a persistent CodingTask through `coding_task_id`. Bound tasks are claimed in the current user/thread TaskGraph before the sub-agent starts and are marked completed only after a successful terminal result. Failed, cancelled, timed-out, missing, or interrupted executions persist a failure reason; recovery resets the task to pending so it can be claimed again. Ordinary `task` calls remain unchanged.

This is how DeerFlow handles tasks that take minutes to hours: a research task might fan out into a dozen sub-agents, each exploring a different angle, then converge into a single report — or a website — or a slide deck with generated visuals. One harness, many hands.

The bundled `multi-agent-coding` skill first persists the stable `coding-analysis -> coding-implementation -> coding-review` DAG with `submit_task_plan`, then passes each stage ID back through `task(coding_task_id=...)` so real delegation results drive the stored workflow.
