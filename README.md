...
- Occurrence ordering applies to rows admitted by upgraded Gateway instances. During a rolling upgrade, rows written by pre-upgrade instances carry no sequence and recovery falls back to the previous timestamp ordering for those rows only; the ordering guarantees hold once every Gateway writer runs the upgraded version. Existing history is not backfilled and keeps best-effort recovery; the upgrade does not reconstruct past order or repair historical counts.
