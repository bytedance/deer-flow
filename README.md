...
- Freeze a task's definition while an occurrence is `queued`, `launching`, or `running`, so a durable occurrence cannot silently pick up a different prompt, thread, or schedule; pause/delete cancel a waiting occurrence, while edits and resume retry after the occurrence finishes
