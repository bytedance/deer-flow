...
For stdio MCP servers, per-tool call timeouts can be configured with `tool_call_timeout`; durable background-task calls honor the same setting for HTTP/SSE servers as well. Both `tool_call_timeout` and `session_init_timeout` accept `null` to disable their bound or a finite number greater than zero; booleans, zero, negative, NaN, and infinite values are rejected by file and Gateway configuration validation, and `tool_call_timeout` is capped at the largest whole-second value Python `timedelta` can represent. When an upgraded installation contains an invalid timeout written by an older Gateway, DeerFlow warns and skips only that MCP server so healthy servers and skills still load. The invalid server is omitted from `GET /api/mcp/config` and Settings > Tools, but unrelated configuration writes such as skill enable-state changes preserve its original raw entry and stored credentials. Because PUT is a full replacement, sending that GET-derived configuration through `PUT /api/mcp/config` still removes the omitted entry and its stored credentials, including `env`, `headers`, OAuth secrets, and per-user credential values in `user_auth.users`. Back up `extensions_config.json` with its secrets before repair, then re-supply the complete server definition from the original file or backup, or edit/restore the file before a full-replacement save. If the stored entry also contains an invalid non-timeout field, repair PUT returns 422 naming that field and leaves the file unchanged; edit the field directly before retrying. Disabling the invalid server remains available through PATCH, while enabling it returns 422 until it is repaired. See [MCP server timeouts](backend/docs/MCP_SERVER.md#upgrading-configurations-written-by-older-gateways) for the full recovery procedure.
