...
When you install `.skill` archives through the Gateway, DeerFlow accepts standard space-separated `allowed-tools`, optional frontmatter metadata, and the Claude-compatible `argument-hint` field instead of rejecting otherwise valid external skills. YAML lists remain supported for `allowed-tools`. Unscoped portable names such as `WebFetch` and `Read` normalize to DeerFlow's `web_fetch` and `read_file` tools. Argument-scoped entries such as `Bash(git:*)` remain inactive because DeerFlow does not inspect tool arguments; declare `bash` only when the skill may use the full Bash tool.
