...
Compaction keeps the current user request and summarizes older assistant/tool activity within `summarization.trim_tokens_to_summarize` (4000 by default). Setting this option to `null` disables input trimming for the summary model; choose that only when the model can accept the full history being compacted.
