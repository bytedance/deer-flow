...
Individual discovered tools can be omitted with `tools.<original_tool_name>.enabled: false`; the key uses the server's original MCP name before DeerFlow's default prefix is added.

OpenViking users can register the official Streamable HTTP endpoint at `/mcp`
with an owner-bound USER API key and keep its destructive `forget` tool
disabled by default. This explicit, model-selected MCP tool path can run
alongside the separate automatic OpenViking memory backend; it does not replace
automatic turn capture or recall. See the
