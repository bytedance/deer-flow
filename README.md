...
The Web composer hides and clears suggestions from the previous turn as soon as the next run starts streaming, preventing stale chips and their close control from overlapping the To-dos panel or input box. User-stopped runs also skip follow-up generation for the incomplete response.
