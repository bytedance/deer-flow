...
The chat header also shows a context-window gauge with the current percentage and a per-category breakdown when the selected model has a positive `context_window` configured. The gauge stays visible as a lightweight placeholder while checkpoint usage is temporarily unavailable, so the control does not jump in and out during refetches or timeouts; cached usage is reused only for the same thread, never after navigating to another chat. Context rendering is bounded to its dedicated workers, so overload falls back to the placeholder instead of accumulating background jobs. With `token_usage.counting: exact`, message text, structured content and citations, replayed reasoning, tool-call names, arguments, IDs, and provider-required thought signatures are tokenizer-counted; image blocks retain a bounded low-resolution estimate instead of counting URLs or base64 data as text.
