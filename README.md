...
DeerMem as the default. Its baseline session snapshot remains enabled by
default; operators can additionally enable request-only recall against each
latest user question with `memory.turn_injection_enabled: true`. The integration
supports one DeerFlow user with one credential-bound OpenViking USER API key in
`memory.mode: middleware` and does not inherit arbitrary HTTP headers from
`ovcli.conf`.
...
Memory injection follows the configured operation mode. In `middleware` mode, DeerMem injects the user-global summaries and the selected agent's facts. In `tool` mode, the automatic `<memory>` block contains only the global `user` and `history` summaries; agent facts are retrieved explicitly through `memory_search`, avoiding duplicate automatic and tool-returned fact context. `memory.injection_enabled` is the master gate; `session_injection_enabled` defaults on for the existing baseline snapshot, while `turn_injection_enabled` defaults off and only applies to query-aware backends. Setting the master gate to `false` disables both paths.
