# edu-flow Sandbox Provisioner

The **Sandbox Provisioner** is a FastAPI service that dynamically manages sandbox Pods in Kubernetes. It provides a REST API for the edu-flow backend to create, monitor, and destroy isolated sandbox environments for code execution.
...
2. **Pod Creation**: The provisioner creates a dedicated Pod in the `edu-flow` namespace with:
...
|----------|---------|-------------|
| `K8S_NAMESPACE` | `edu-flow` | Kubernetes namespace for sandbox resources |
...
3. **Kubernetes Access**:
   - The provisioner needs permissions to:
     - Create/read/delete Pods in the `edu-flow` namespace
     - Create/read/delete Services in the `edu-flow` namespace
     - Read Namespaces (to create `edu-flow` if missing)
...
docker compose -p edu-flow-dev -f docker/docker-compose-dev.yaml up -d provisioner
```
...
docker exec edu-flow-provisioner curl -X POST http://localhost:8002/api/sandboxes \
  -H "Content-Type: application/json" \
  -d '{"sandbox_id":"test-001","thread_id":"thread-001"}'

# Check sandbox status
docker exec edu-flow-provisioner curl http://localhost:8002/api/sandboxes/test-001

# List all sandboxes
docker exec edu-flow-provisioner curl http://localhost:8002/api/sandboxes

# Verify Pod and Service in K8s
kubectl get pod,svc -n edu-flow -l sandbox-id=test-001

# Delete sandbox
docker exec edu-flow-provisioner curl -X DELETE http://localhost:8002/api/sandboxes/test-001
```
...
SANDBOX_URL=$(docker exec edu-flow-provisioner curl -s http://localhost:8002/api/sandboxes/test-001 | jq -r .sandbox_url)

# Test from gateway container
docker exec edu-flow-gateway curl -s $SANDBOX_URL/v1/sandbox
```
...
- Verify inside container:
  ```bash
  docker exec edu-flow-provisioner ls -ld /root/.kube/config
  ```
...
- Verify the paths exist on your host machine:
  ...
  ls -la /path/to/backend/.edu-flow/threads
  ```
...
- Check Pod events: `kubectl describe pod sandbox-XXX -n edu-flow`
...
**Solution**:
- Verify the Service exists: `kubectl get svc -n edu-flow`
...
3. **Network Isolation**: Sandbox Pods run in the `edu-flow` namespace but share the host's network namespace via NodePort. Consider NetworkPolicies for stricter isolation.
