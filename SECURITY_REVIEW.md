...
**Finding:** Stored Cross-Site Scripting (XSS) in artifact rendering path  
**OWASP Top 10:2025 Mapping:** **A03 Injection**  
**Credibility:** **High** (estimated true-positive confidence: **0.92 / 1.00**)

The artifacts endpoint serves `text/html` artifacts using `HTMLResponse` with raw file content. If attacker-controlled HTML is written to an artifact (for example by prompt-driven output generation), the payload can execute in the application origin when a user opens that artifact link.
