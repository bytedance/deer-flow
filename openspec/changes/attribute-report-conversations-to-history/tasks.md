...
- [ ] 2.4 更新 `report-runs-page.test.tsx` 覆盖新 Tab 的交互
...
- [ ] 3.5 更新侧边栏相关测试（`sidebar.spec.ts` E2E 或单元测试）

## 4. 验证与收尾

- [x] 4.1 运行 `pnpm check` 确保类型和 lint 通过
- [x] 4.2 运行 `pnpm test` 确保单元测试通过
- [ ] 4.3 手动验证：从侧边栏"报告历史"进入 → 切换到"对话" Tab → 点击对话跳转 → 面包屑可回到报告历史
- [ ] 4.4 手动验证：侧边栏"报告对话"子列表正确展示、折叠、跳转
