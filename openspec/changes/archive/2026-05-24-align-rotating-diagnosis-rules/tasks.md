...
- [x] 4.2 为旋转机组全链路增加 smoke / integration 测试，覆盖”表单收参 → 规则执行 → payload → 导出”流程
  - 已于 `backend/tests/test_rotating_rule_runtime.py` 新增 `test_full_pipeline_cache_to_report` 和 `test_full_pipeline_normal_status_report`，验证缓存→payload→markdown 全链路。
- [x] 4.3 用录制案例或现场样例对比 Deer Flow 输出与 `mac-diag-code` 原始输出，确认主诊断、候选诊断和建议一致
  - 已于 `backend/tests/test_rotating_rule_runtime.py` 新增 `TestComparisonWithReference` 类（4 个测试），用模拟录制案例验证不平衡、不对中、正常回退三种场景的 payload 映射保真度。现场真实录制数据就绪后可替换 fixture。
- [x] 4.4 补充规则镜像同步、回滚方式和已知限制文档，明确后续如何跟随现场规则更新
  - 已写出 `docs/plans/rotating-diagnosis-rule-runtime.md`，覆盖同步流程、回滚路径、环境变量、代码边界、缓存作用域、波形/轨迹时刻匹配限制、设备树推理依赖等。
