...
### 9. 会话恢复窗口内的新请求需要等待恢复完成

当前实现虽然把多个 `401` 归并到了同一个恢复 promise，但仍允许恢复窗口内新发起的业务请求继续直接打到 DeerFlow 网关。

这会导致：

- 第一个请求触发 `401`
- 宿主 refresh 已开始
- `/api/v1/auth/ins-base/authenticate` 尚未完成前
- 其余并发请求继续命中旧 DeerFlow session，产生一串额外 `401`

因此当 EHM session recovery 已在进行中时，新的 DeerFlow 业务请求应先等待恢复 promise 完成，再继续发起请求。
...
当前 `fetcher` 在最终 401 分支里直接挂一个延迟跳转到 `/login` 的定时器。
...
- 后续恢复链路实际上已经成功
- 但旧定时器仍会在 2 秒后执行
- 页面先跳到 `/login`，再被桥接带回工作台，产生明显闪烁

因此延迟跳转必须具备以下能力：

- 全局唯一
- 恢复成功后可取消
- 真正执行前再次检查当前是否仍在恢复，或是否已有更新的恢复成功信号

实现上不建议通过“持续给同一个 timer 续期”解决，而应在恢复中直接阻止本次跳转，待恢复最终失败后再重新安排跳转。

### 11. Bridge 侧 session 重建成功需要向全局广播恢复信号

当前 DeerFlow 有两条会话恢复入口：

- `fetcher.ts` 里的 `recoverEhmSession()`
- `ehm-host-bridge.ts` 收到 `AI_TOKEN_REFRESH` 后立即调用 `/api/v1/auth/ins-base/authenticate`

如果只有 `fetcher.ts` 自己知道恢复成功，而 bridge 侧恢复成功没有向全局广播，那么：

- 某个较早失败的请求可能已经挂起登录跳转
- bridge 侧实际上已经重建好了 DeerFlow session
- 但 `fetcher.ts` 不知道这次“外部恢复”已经成功
- 旧跳转仍可能继续执行

因此 bridge 侧在 session 重建成功后必须向全局广播恢复成功信号，使 `fetcher.ts` 能统一取消旧的登录跳转与失败态。

### 12. 首次 401 只负责触发恢复，不直接判定整页登录失败

当前剩余的闪烁现象说明，仅仅把恢复链路补齐还不够。

如果第一次业务请求（例如 `todo-stats`）在旧 DeerFlow session 下先返回 `401`，而这个请求本身就立即具备“安排登录跳转”的资格，那么即使：

- 宿主 refresh 已成功
- bridge 已经拿到新 `ehm_token`
- `/api/v1/auth/ins-base/authenticate` 很快也会成功

页面仍可能被第一次 `401` 过早送去 `/login`，随后再被桥接带回工作台，形成明显闪烁。

因此对于存在 `ehm_token` 的 iframe 会话，首次 `401` 应降级为“恢复触发器”：

- 它负责启动 `recoverEhmSession()`
- 若恢复失败，该请求本身可以继续向上抛出 `401`
- 但它不应直接安排整页登录跳转

只有在后续请求再次确认系统仍处于未恢复状态时，才允许真正进入登录跳转。

### 13. 业务层 401 兜底跳转也必须感知恢复窗口

即使 `fetcher.ts` 已经避免首次 `401` 直接安排登录跳转，若业务层在捕获错误后仍通过统一 `api-error` 工具立刻安排一次 `/login` 跳转，最终效果仍然相同。

因此业务层的认证错误兜底也必须共享同一条判定规则：

- 当 iframe EHM 恢复仍在进行中
- 或当前请求仍处于“首次 401 仅触发恢复”的观察窗口

业务层不应再次单独触发登录跳转，而应把控制权交回全局恢复状态。
...
- 恢复窗口内新发起的业务请求会等待恢复完成，而不是继续雪崩式触发额外 401
- 已经恢复成功时，不会再被较早挂起的旧登录跳转定时器带去 `/login`
- 通过宿主 bridge 完成的 session 重建也会被视为全局恢复成功信号
- 首次 `401` 不会因为过早触发整页登录跳转而把 iframe 先带到 `/login`
- 业务层的统一认证错误处理不会绕过恢复窗口再次安排重复登录跳转
