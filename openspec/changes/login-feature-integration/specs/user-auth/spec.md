...
**Reason**: 对接 ins-base-rpc 后，用户统一由 Java 端管理注册，本地注册不再需要

**Migration**: 用户注册请通过 ins-base-rpc 的管理后台进行操作

#### Scenario: 用户通过 /api/v1/auth/register 注册
- 该端点已被移除，请求会收到 404
- `/api/v1/auth/register` 已从所有中间件的公共路径白名单中移除
