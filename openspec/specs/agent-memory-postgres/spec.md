...
Agent Memory cross-worker sharing via PostgreSQL-backed `StoreMemoryStorage` for multi-worker deployments. When multi-worker mode is active, agent memory is stored in the shared PostgreSQL `store_items` table instead of local files, with optimistic merge for concurrent writes.
