# frontend-i18n-completeness Specification

## Purpose

Ensure all user-facing strings in the DeerFlow frontend — across report template pages, report run pages, and GenUI dynamic blocks — are fully internationalized using the `useI18n()` hook, with no hardcoded Chinese remaining in production component code.
