# builtin-report-direct-executor Specification

## Purpose

Provides a direct execution path for builtin tri-reports (daily, weekly, monthly) that bypasses the DSL template engine state machine. The `report_direct_execute` tool directly orchestrates Skill script calls (`query_*.py` -> `*_kpi.py` -> `export_report.py`) to produce report artifacts without intermediate form steps.
