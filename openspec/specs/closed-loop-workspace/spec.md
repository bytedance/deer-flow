# closed-loop-workspace Specification

## Purpose

定义前端工作台中「闭环管理」模块的导航入口、列表/看板视图、详情抽屉、处置与验证表单、待办徽标，以及状态迁移成功后的乐观更新行为。本能力以 `closure:read` 权限点和 feature flag `NEXT_PUBLIC_CLOSED_LOOP_ENABLED` 双重控制可见性。
