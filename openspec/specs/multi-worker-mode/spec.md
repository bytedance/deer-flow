...
Opt-in multi-worker deployment mode that automatically switches all stateful components to shared backends (PostgreSQL + Redis). Controlled via `deployment.mode` config or `DEER_FLOW_MULTI_WORKER=1` env var, with explicit overrides taking precedence.
...
- **THEN** the system SHALL switch all stateful components to shared backends:
  - `database.backend` to `postgres`
  - `stream_bridge.type` to `redis`
  - `rag.vector_store_backend` to `pgvector`
  - `rate_limit.backend` to `redis`
  - `cost.storage_backend` to `postgres`
  - `indexing.dispatcher_mode` to `queue`
  - `im.coordination_mode` to `redis`
  - `memory.storage_class` to `StoreMemoryStorage`
...
- **THEN** the existing auto-default logic SHALL apply (memory.storage_class to StoreMemoryStorage, rag to pgvector, cost to postgres)
