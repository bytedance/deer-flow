# closed-loop-tickets Specification

## Purpose

定义「闭环单（Closure Ticket）」的领域模型、状态机、SLA、检索查询能力与权限模型。闭环单是设备问题/整改项在 deer-flow 内的唯一聚合根，由诊断 Agent、报告 Agent 等来源自动或人工创建，通过受控状态机推进至闭环完成。
