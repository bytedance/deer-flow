...
Weekly report direct execution path that bypasses the DSL template engine state machine. The `DirectReportExecutor` orchestrates `query_weekly.py` → `weekly_kpi.py` → `export_report.py` with performance instrumentation, concurrent InS fetching, and equipment metadata passthrough.
