...
Specifies the optional `provider` field on the DSL `DataStep` schema, which controls how the `data_runner` selects the data acquisition path for each step. Also specifies the per-report-type skill namespace routing for DSL template `name:` fields and Agent SOUL.md script paths.
