# postgres-checkpoint-cleanup

## Purpose

Ensure PostgreSQL-backed checkpointer data is fully cleaned up when a thread is deleted, preventing orphaned checkpoint rows from accumulating in `checkpoints`, `checkpoint_writes`, and `checkpoint_blobs` tables.
