...
4. 读取脚本输出的 `equipment` 列表，按 `equipment_kind` 过滤。如果脚本未返回 `kind` 字段，按设备 `name` 关键字粗略匹配（"汽轮" / "离心" / "轴流" / "齿轮" / "螺杆"），未识别的归入"未分类"分组。
...
```json
{
  ...
  "props": {
    ...
    "fields": [
      {
        ...
        "max_visible": 10,
        "options": [
          {"label": "K-101", "value": "K-101", "group": "A 区", "description": "丙烯压缩机"},
          {"label": "ST-201", "value": "ST-201", "group": "B 区", "description": "驱动汽轮机"}
        ]
      },
      {
        "name": "key_points",
    ...
    "default_values": {
      "equipment_ids": ["K-101", "K-102", "K-103", "ST-201", "GB-301"],
      "key_points": [
        "vib_de_x", "vib_de_y", "vib_nde_x", "vib_nde_y",
        "axial_displacement", "bearing_temperature", "thrust_bearing_temperature",
        "speed", "inlet_flow", "anti_surge_valve_opening"
      ]
    },
...
> **测点裁剪**：默认列出全 10 项；如目标设备 InS 树中没有某测点（例如螺杆压缩机通常无 X/Y 双探头轴振，齿轮箱通常无防喘振阀），LLM 在 Round 1.5 渲染前应通过 `ins-device-analysis` 探测一次设备组件树，把缺失测点从 `options` 与 `default_values.key_points` 中剔除。如未做探测，保留默认列表，由后续脚本自然降级。
...
**从对话历史中回溯找到"当前消息之前最近一次"的 `callback_id=fd-rotating-scope` 和 `callback_id=fd-rotating-target` 的 `ui_interaction` 消息**，分别提取参数：
