...
- **[backend/CLAUDE.md](backend/CLAUDE.md)** — backend depth: harness/app split, agent &
  middleware chain, sandbox, MCP, skills, memory, IM channels, persistence/migrations,
  TUI, config system, test layout.
...
[backend/CLAUDE.md](backend/CLAUDE.md) for the runtime and router detail.
...
├── frontend/                       # Next.js frontend (pnpm) — see frontend/CLAUDE.md
...
```bash
# Backend (see backend/CLAUDE.md for the full set)
cd backend && make dev        # Gateway API with reload (port 8001)
cd backend && make test       # Backend test suite
cd backend && make lint       # ruff check
cd backend && make format     # ruff format

# Frontend (see frontend/CLAUDE.md for the full set)
...
- **Documentation update policy** — keep docs in sync with code: update `README.md` for
  user-facing changes and the relevant `CLAUDE.md` for development/architecture changes in
  the same change set.
- **Test-driven development** — features and bug fixes ship with tests. Backend tests live
  in `backend/tests/` (TDD is mandatory there; see [backend/CLAUDE.md](backend/CLAUDE.md));
