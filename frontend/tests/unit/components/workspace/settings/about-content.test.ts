...
test("aboutMarkdownEnUS heading interpolates the app version", async () => {
  process.env.NEXT_PUBLIC_APP_VERSION = "9.9.9-test";
  const { aboutMarkdownEnUS } =
    await import("@/components/workspace/settings/about-content");
  // The heading link text carries the version stamp.
  expect(aboutMarkdownEnUS).toContain("[About DeerFlow 9.9.9-test]");
  // Milestone copy in the acknowledgments refers to the 1.0/2.0 product
  // generations and must NOT be parameterized.
  expect(aboutMarkdownEnUS).toContain("DeerFlow 1.0 and 2.0");
});

test("aboutMarkdownEnUS heading reflects the package version when env is unset", async () => {
  delete process.env.NEXT_PUBLIC_APP_VERSION;
  ...
  expect(aboutMarkdownEnUS).toContain(`[About DeerFlow ${APP_VERSION}]`);
});
