...
- **Reduced motion is handled at the call site.** `components/ui/**` is generated from registries and not hand-edited, so a decorative component there is wrapped rather than modified: gate it with `useRenderActivity` (which already drops animation under `prefers-reduced-motion`), pass the component's own opt-out prop, or feed it a value that freezes it. Prefer freezing over unmounting — pass `useRenderActivity`'s third argument as `false` when the content must still render, as Magic Bento and the skills walkthrough do, so reduced motion never leaves an empty section. `globals.css` carries a `prefers-reduced-motion` rule only for `animate-aurora`, whose registry component applies it unconditionally; keep such rules scoped to one animation, since entrance keyframes like `fade-in-up` start at `opacity: 0` and a blanket `animation: none` would hide content.
