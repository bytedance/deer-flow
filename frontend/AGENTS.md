...
- **Run stream options** are sanitized by `core/api/stream-mode.ts`: stream modes are filtered to the Gateway-supported set (`values`, `messages`, `messages-tuple`, `updates`, `debug`, `tasks`, `checkpoints`, `custom`), requests with no supported mode remaining throw before HTTP instead of silently defaulting to `values`, and `streamResumable` is retained by thread hooks only for SDK-side reconnect bookkeeping but stripped before the HTTP request because the Gateway does not implement resumable SSE. Keep this boundary aligned with the backend request schema; `events` is not supported and must not be forwarded.
