...
The workspace's offline-Gateway recovery probe owns a bounded `/auth/me` request on each polling attempt. A hung probe must time out before the next interval so `inFlight` cannot permanently suppress recovery, and effect cleanup must abort the owned request without updating an unmounted banner.
