...
3. Stop actions call the LangGraph SDK stream stop path; `core/threads/hooks.ts` invalidates current-thread, token-usage, and sidebar/search caches because SDK stop may finish via abort + fire-and-forget cancel without an `onFinish` event
