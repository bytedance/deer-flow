...
Follow-up suggestions are previous-turn UI state. `InputBox` must synchronously exclude them from rendering whenever `status === "streaming"`, then clear their chips/loading state when that stream begins; the post-stream effect may repopulate suggestions only for the newly completed AI message. A user-stopped stream is marked before cancellation and must skip that post-stream generation because its AI response is incomplete. This prevents a stale row and its dismiss control from overlapping the To-dos panel or composer (#3395).
