...
Follow-up suggestions are previous-turn UI state. `InputBox` must render them only when `status === "ready"`, synchronously clear their chips/loading state when a new stream begins, and repopulate them only after a successfully completed AI message. User-stopped and failed streams must skip post-stream generation because their AI responses are incomplete. This prevents a stale row and its dismiss control from overlapping the To-dos panel or composer (#3395).
