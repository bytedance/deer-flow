...
Admin user management lives in `Settings > Users` and is exposed only to a real signed-in administrator, not regular users or the auth-disabled/static synthetic admin identities. The backend rejects the management API entirely while authentication is disabled, including when a browser retains an older real session cookie. The page lists existing local and OIDC accounts from `GET /api/v1/admin/users?offset&limit` and promotes or demotes them through `PATCH /api/v1/admin/users/{id}/role`; role authority remains server-side. After an actual role change, the affected user's old cookie is rejected at the next authentication boundary and the normal sign-in flow must recover the UI. Do not model this as immediate revocation of an already-open stream or other long-lived connection—the backend does not actively interrupt those connections.
