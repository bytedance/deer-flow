...
The `?mode=skill` new-chat initializer delays its prompt write briefly so the
composer can mount. That timer is owned by the hook effect and must be cleared
on route change or unmount; only mark the prompt as applied when the timer
actually fires, so a cancelled navigation can initialize correctly if the user
later re-enters skill mode.
