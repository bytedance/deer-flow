...
- **Subtask render synchronization** (`core/tasks/subtask-render.ts`) — `MessageList` derives stable task snapshots from grouped messages in `useMemo` and synchronizes them into task context from an effect, never during render. Terminal message snapshots are authoritative for status/result/error while retaining fields available only from live SSE context; a metadata-only live skeleton is overlaid on the complete non-terminal message snapshot so cards never lose their description or status while the first subagent step is pending.
