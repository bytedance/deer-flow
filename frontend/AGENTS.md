...
- **Streaming Markdown animation** is owned by Streamdown's `animated` / `isAnimating` API through `core/streamdown`; do not reintroduce a rehype plugin that wraps every word, because reparsing a growing block remounts old words and replays their animation.
