...
The OIDC callback owns both its `/auth/me` `AbortController` deadline and its delayed redirect timer. Route changes and unmounts must abort the request and cancel the redirect so an obsolete callback page cannot navigate a newer route.
