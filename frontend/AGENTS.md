...
Single-run schedule edits retain the mounted task's original `run_at` while its wall time and timezone match. The parent echoes edits through `initial`; retain a stable snapshot and remount with a task key when switching tasks. Component and scheduled-task E2E tests cover DST folds and timestamp precision.
