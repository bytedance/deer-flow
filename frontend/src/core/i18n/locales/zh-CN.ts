...
export const zhCN: Translations = {
  ...
  settings: {
    ...
    modelServices: {
      title: "模型服务",
      description:
        "按提供商而不是按模型类型查看配置。一个提供商可以同时承载文本、图片、视频和音频服务。",
      providersTitle: "提供商",
      providersDescription:
        "这里会把后端已配置的模型按 provider 聚合，便于统一管理多模态能力。",
      searchPlaceholder: "搜索提供商或模型...",
      emptyTitle: "没有找到提供商",
      emptyDescription:
        "先添加一个提供商，再统一管理它的文本、图片、视频和音频模型。",
      newProvider: "新提供商",
      newModel: "新模型",
      saveChanges: "保存修改",
      saving: "保存中...",
      addProvider: "添加提供商",
      addModel: "添加模型",
      defaultModelsTitle: "全局默认模型",
      defaultModelsDescription:
        "当聊天或后续多模态工具没有显式指定模型时，会优先使用这里的默认模型。",
      defaultTextModel: "默认文本模型",
      defaultImageModel: "默认图片模型",
      defaultVideoModel: "默认视频模型",
      defaultAudioModel: "默认音频模型",
      basicInfoTitle: "基础信息",
      basicInfoDescription: "管理提供商档案、支持模态和启用状态。",
      apiConfigTitle: "API 配置",
      apiConfigDescription: "配置该提供商的 Base URL、API Key 和自定义请求头。",
      advancedTitle: "能力覆盖",
      advancedDescription: "对比 provider 级能力与当前已启用模型实际覆盖的能力。",
      connectionTitle: "连接测试",
      connectionDescription:
        "会先保存当前 provider，再测试 `/models` 和最小化的 `chat/completions` 请求。",
      providerName: "提供商名称",
      providerType: "提供商类型",
      providerHomepage: "主页地址",
      providerNotes: "备注",
      providerModalities: "提供商模态",
      enabled: "启用",
      baseUrl: "Base URL",
      apiKey: "API Key",
      apiKeyHint: "留空表示保留当前密钥；如果要删除，请显式点击清空。",
      clearApiKey: "清空密钥",
      headersJson: "Headers JSON",
      headersHint: "请输入 JSON 对象，例如 {\"X-Title\": \"DeerFlow\"}。",
      discoverModels: "获取模型列表",
      discoveringModels: "正在获取模型...",
      discoverModelsTitle: "发现远端模型",
      discoverModelsDescription:
        "查看当前 provider 的远端 `/models` 列表，然后选择要导入到本地草稿里的模型。",
      discoverModelsSearchPlaceholder: "搜索远端模型...",
      discoverModelsEmpty: "这个 provider 没有匹配到可导入的模型。",
      importSelectedModels: "导入选中模型",
      selectedModelsCount: (count: number) => `已选择 ${count} 个模型`,
      selectedForImport: "待导入",
      alreadyImported: "已导入",
      modelSource: "来源",
      syncModels: "同步模型",
      testConnection: "测试连接",
      ...
      extraBodyHint: "可选 JSON，会并入请求体，适合填写网关或 provider 特殊参数。",
      noModels: "这个提供商下还没有配置任何模型。",
      noProviderSelected: "请选择一个提供商",
      connectionEmpty: "执行一次连接测试后，这里会展示最新结果。",
      connectionOk: "连接成功",
      connectionFailed: "连接失败",
      configuredStatus: "已配置",
      serviceCoverage: "服务覆盖",
      serviceEnabled: "当前模型配置已覆盖",
      serviceAvailable: "提供商侧可用",
      serviceNotConfigured: "当前模型配置未覆盖",
      configSourceTitle: "配置来源",
      configSourceLabel: "当前由",
      configSourceDescription:
        "这里展示的是从后端模型配置聚合出来的 provider 视图。API Key、Base URL 等仍然由 config.yaml 和环境变量管理。",
      openProvider: "打开提供商",
      modelsTitle: "已配置模型",
      modelsDescription:
        "模型被收拢在各自提供商下，方便把文本、图片、视频、音频能力放在一起管理。",
      modelsCountSuffix: "个模型",
      systemModelsTitle: "系统已接入模型",
      systemModelsDescription:
        "这些模型已经通过 config.yaml 接入系统，这里会按当前 provider 动态展示出来。",
      createManagedProvider: "创建可管理配置",
      createManagedProviderDescription:
        "这个 provider 目前只存在于运行时模型注册表里。如果你想在这里编辑 Base URL、API Key、Headers，或者继续添加 provider 自有模型，可以先创建一个可管理的 provider 配置。",
      sourceConfig: "config.yaml",
      sourceProvider: "Provider 配置",
      thinking: "思考",
