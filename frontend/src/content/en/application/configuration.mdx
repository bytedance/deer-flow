...
Set these in the DeerFlow process — via a `.env` file, the Docker environment, or the shell:

| Variable                | Default         | Description                                               |
| ----------------------- | --------------- | --------------------------------------------------------- |
| `DEER_FLOW_CONFIG_PATH` | auto-discovered | Absolute path to `config.yaml`                            |
| `DEER_FLOW_ROOT`        | repository root | Base directory used for skill and thread mounts in Docker |
