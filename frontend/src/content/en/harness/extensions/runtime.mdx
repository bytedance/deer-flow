...
A missing marker skips the check. Otherwise the rule before 1.0 is: same major and minor as the host, and a patch no newer than the host's. From 1.0 on it will be: same major, and a minor and patch no newer than the host's. With the host at `0.2.3`:

| `@extension(api=...)` | Result                                                                          |
| --------------------- | ------------------------------------------------------------------------------- |
| not decorated         | Loaded                                                                          |
| `"0.2"`, `"0.2.0"`, `"0.2.3"` | Loaded                                                                  |
| `"0.2.4"`             | Refused: the extension may use additions this host lacks                         |
...
```text
Extension acme_audit:install: extension requires extension-api 0.2.4, host provides 0.2.3. Install a matching version: pip install 'deerflow-extension-api>=0.2.4,<0.3'
```
