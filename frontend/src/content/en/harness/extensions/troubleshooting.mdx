...
- Declared version **newer** than the host (for example `0.3.0` on a `0.2.3` host): upgrade DeerFlow, or install an extension release built for this host.
- Declared version with a **newer patch** than the host (for example `0.2.4` on a `0.2.3` host): upgrade DeerFlow, or install the extension release that declares `0.2.3` or lower.
- Declared version on an **older** minor (for example `0.1.0` on a `0.2.3` host): upgrade the extension to a release written against the host's minor.
