...
Configured middleware is loaded into both lead-agent and subagent pipelines after the built-in middleware and optional loop/token guards, but before the terminal-response, safety, and clarification tail. Treat these class paths as trusted configuration because loading one executes Python code. Embedded callers can instead use `DeerFlowClient(middlewares=[AuditMiddleware()])` or `create_deerflow_agent(extra_middleware=[AuditMiddleware()])`.
