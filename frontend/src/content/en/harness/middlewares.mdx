...
The ordering is significant. Durable context capture runs before summarization so completed delegation results and loaded skill references survive compaction. Clarification always runs last so it can intercept after all other middlewares have had their turn.
...
Captures long-lived runtime facts into explicit thread-state channels before summarization compacts the raw transcript. It records compressed summaries, completed delegation results, and loaded `SKILL.md` references, then projects them into later model calls as hidden durable context data.
