...
For operator-managed deployments, register a class by import path. A string uses the zero-argument constructor; `{class, kwargs}` passes constructor arguments:

```yaml
extensions:
  middlewares:
    - my_company.deerflow_middlewares:MyMiddleware
    - class: my_company.deerflow_middlewares:MyMiddleware
      kwargs:
        max_tool_calls: 5
```
