...
Custom agents are created through the DeerFlow App UI or via the `/api/agents` endpoint. Both paths require `agents_api.enabled: true` in `config.yaml`; the management API is disabled by default. Configuration is stored per user at `{data_dir}/users/{user_id}/agents/{name}/config.yaml`, where `{data_dir}` is `DEER_FLOW_HOME` (default: `backend/.deer-flow`).
