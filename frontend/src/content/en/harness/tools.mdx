...
<Tabs items={["DuckDuckGo (default)", "Tavily", "Brave", "Exa", "InfoQuest", "Firecrawl", "GroundRoute", "You.com"]}>
  <Tabs.Tab>
```yaml
tools:
...
tools:
  - use: deerflow.community.youcom.tools:web_search_tool
    # api_key: $YDC_API_KEY # optional; omit to use the keyless free tier
```
Results from You.com's own index, each carrying extracted page snippets rather than a
single meta description. Without a key it uses the keyless endpoint (free tier, currently
100 searches/day); set `YDC_API_KEY` and it switches to the full Search API with higher
limits. `max_results` is capped at 100.

Optional API key from [You.com](https://you.com/platform?utm_source=bytedance-deer-flow&utm_medium=oss_integration&utm_campaign=2026-08-oss-integrations&utm_content=docs).
