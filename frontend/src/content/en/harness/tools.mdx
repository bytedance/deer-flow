...
Add `x_search` alongside `web_search` when an agent needs structured X/Twitter
post results rather than general web-index matches:
...
The tool supports chronological `Latest` and engagement-ranked `Top` search.
Results include post text, author handles, timestamps, URLs, and engagement
metrics. It requires an [Xquik](https://xquik.com) API key and adds no extra
dependency. The result limit is capped at 100.
