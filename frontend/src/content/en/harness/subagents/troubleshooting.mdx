...
The turns ran out and the last assistant message had no usable text. Same fix as above.
...
**Cause**: subagents shared the AIO implicit shell session, or the session count exceeded the image limit and sessions were evicted. [#5134](https://github.com/bytedance/deer-flow/pull/5134) gives every subagent its own execution lease and persistent session, and [#5178](https://github.com/bytedance/deer-flow/pull/5178) sets `MAX_SHELL_SESSIONS` to `max_running + 1` automatically.
