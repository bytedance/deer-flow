...
| Guard configuration: `summarization`, `loop_detection`, `token_budget`        | Parent callbacks bound to the parent event loop. Token usage and audit events reach the parent through proxies     |
| The request trace id and the IM `channel_user_id`                             | The `task`, `ask_clarification`, and `present_files` tools. No further delegation, no questions to the user        |
