...
| `failed`              | The model call failed after retries, capacity was rejected, or a runtime error occurred        | Failed   | `Task failed.`                            |
...
Failure-class statuses append `Error: <detail>` when a detail exists. The text body is display content only; the frontend and the Lead Agent rely on the structured fields in the message metadata, not on parsing text.
...
Three guards can end a run early while it still counts as `completed`:
...
A capped report is a **partial result**: the tool calls of the final turn were stripped and the model was forced to answer. When the turn budget runs out with no usable text at all, the status is `failed` with the error `Reached max_turns=N`. The legacy `max_turns_reached` status maps to `turn_capped` on read.
...
| `no_citation_claims`  | The report cites nothing but contains action verbs or paths, or is at least 240 characters long             |
...
- [holds] file:outputs/report.md non-empty — 1,204 bytes
...
- [completed] research competitors (via general-purpose; execution finished; retain useful work; repair/recheck unmet criteria; ...) -> Pricing of the top 5 competitors… · citations: 3 resolved · acceptance: 1 hold
