...
- **A dedicated event loop**: every subagent runs on a daemon thread named `subagent-persistent-loop`, separate from the Gateway request loop. Crossing that boundary strips the parent callbacks bound to the original loop (such as the billing journal); token usage and audit events reach the parent through dedicated proxies.
