...
| ------------------------- | ------------------------------------------------------------------------------ | ----------------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Per-response concurrency  | request context `max_concurrent_subagents`                                     | 3                                                                             | 1 to 64, capped by `subagent_runtime.max_running` | Excess `task` calls are dropped and only logged                                                                       |
| Per-run total             | `subagents.max_total_per_run`; request context `max_total_subagents` overrides it | 6                                                                          | 1 to 50                                        | Excess calls are dropped, `[SUBAGENT LIMIT REACHED] ...` is appended to the assistant message, the run's `stop_reason` becomes `subagent_limit_capped` |
...
- **Per-response concurrency**: how many `task` calls one model response may contain, default 3, capped by the process capacity `max_running`. Excess calls are dropped with no text appended, only a log line. The HARD LIMITS line in the prompt tells the model the number.
- **Per-run total**: the cumulative number of delegations in one run, default 6, which is two full batches at the default concurrency. Only ledger entries tagged with the current `run_id` count; without a `run_id` the middleware logs a warning and counts the whole thread. When the total is exhausted, the remaining `task` calls are removed and the assistant message gets this appended:
...
- **Token budget** (`TokenBudgetMiddleware`): tracks the run's cumulative tokens against `subagents.token_budget`. At the hard-stop threshold it strips the current turn's tool calls and forces a final answer, marking the result `completed` + `token_capped`. Reaching `warn_threshold` logs a warning.
