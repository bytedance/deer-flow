...
| `subagents.token_budget.warn_threshold`      | 0.7                                         | Logs a warning at this fraction                                             |
...
| `allowed_subagents`        | From the Custom Agent definition: `null` all, `[]` none, a list as allowlist; snapshotted into run metadata at run start |
...
Result text formats: `Task Succeeded. Result: ...`, `Task Succeeded (capped: <label>). Result: ...`, `Task failed.`, `Task cancelled by user.`, `Task timed out.`, `Task polling timed out after N minutes. ...`; failure-class results may append `Error: <detail>`.
...
| Route                                                            | Purpose                                                        |
| ---------------------------------------------------------------- | -------------------------------------------------------------- |
| `GET/POST /api/subagents`, `PUT/DELETE /api/subagents/{name}`    | Managed subagent catalog, administrators only                  |
...
- [#5178](https://github.com/bytedance/deer-flow/pull/5178) Set AIO `MAX_SHELL_SESSIONS` from `max_running + 1`.
