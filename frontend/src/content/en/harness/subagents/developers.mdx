...
- When the supplied `SubagentRuntime` owns a durable batch service, `batch_task`, `batch_status`, and `cancel_batch` bind to it; otherwise they fall back to the process-global submitter. An owned batch worker that has not been started raises at build time.
...
The backend writes metadata with `make_subagent_additional_kwargs` and reads it with `read_subagent_result_metadata`; an invalid status raises `ValueError`. The frontend's `parseSubtaskResult` reads structured fields first and falls back to text prefixes only when no structured metadata exists at all. Additive fields (model name, token usage, receipts, acceptance) do not require a contract version bump.
