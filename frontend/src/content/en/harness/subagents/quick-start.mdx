...
- DeerFlow is configured per [Install](/docs/harness) and you can chat normally. The `subagents:` section in `config.yaml` can stay empty; the built-in defaults work.
- To use the `bash` subagent you need a container sandbox, or `sandbox.allow_host_bash: true` under the local sandbox, which is only appropriate in a fully trusted local environment. Otherwise the catalog only contains `general-purpose`.
...
If you are talking to a Custom Agent created in Settings, its own subagent access setting may narrow or disable delegation further. See [Subagent Catalog](/docs/harness/subagents/catalog).
