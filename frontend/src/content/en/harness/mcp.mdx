...
- A value that cannot travel as an HTTP header — a stray newline picked up when
  ...
  Latin-1 — is always rejected, regardless of `on_missing`. The error names the
  offending key but never repeats the value; without this check the HTTP
  client's own rejection would echo the full credential into a model-visible
  tool error.
