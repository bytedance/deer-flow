...
The allowlist is copied into run metadata when a run starts and is enforced again by the `task` tool, so a client cannot bypass it by naming a hidden subagent directly. Runtime precedence is **built-in → `config.yaml` → managed → user-scoped Custom Agents**. User Custom Agents carry their SOUL, tool-group allowlist, skills, and model selection when invoked through `task(subagent_type=...)`. If an operator later adds a conflicting `config.yaml` name, the managed or user-scoped entry remains visible in its management surface but is excluded from runtime discovery. Explicit `subagents.agents.<name>` overrides continue to take priority and are shown in the Settings catalog.
