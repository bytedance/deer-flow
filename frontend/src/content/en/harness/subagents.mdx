...
1. Looks up the subagent configuration from the registry, including built-ins, `config.yaml` custom subagents, managed definitions, and the current user's Custom Agents.
...
The allowlist is copied into run metadata when a run starts and is enforced again by the `task` tool, so a client cannot bypass it by naming a hidden subagent directly. Runtime precedence is **built-in → `config.yaml` → managed → user-scoped Custom Agents**. User Custom Agents carry their SOUL, skills, model selection, and a `tool_groups` policy expanded into a strict tool-name allowlist when invoked through `task(subagent_type=...)`; delegated agents always deny `task`, `ask_clarification`, and `present_files`. If an operator later adds a conflicting `config.yaml` name, the managed or user-scoped entry remains visible in its management surface but is excluded from runtime discovery. With file storage, legacy agents under the shared `{base_dir}/agents/` layout remain visible to every user until an operator assigns them with `backend/scripts/migrate_user_isolation.py`; current-layout files and database rows remain owner-scoped. Explicit `subagents.agents.<name>` overrides continue to take priority and are shown in the Settings catalog.
