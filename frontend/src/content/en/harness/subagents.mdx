...
A custom agent dispatched as a subagent keeps its identity: its SOUL becomes the subagent's system prompt, its tool-group allowlist restricts which tools it can use (an unrestricted agent inherits the full pool), and its skills and model selection carry over. Subagents never re-delegate (`task` is always denied for them), and one user's agents are never visible to another user's threads.
