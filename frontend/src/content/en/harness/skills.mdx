...
enabled, user-visible, and Agent-allowed skills; shell commands and file tools
cannot read an omitted skill through its old shared path. Local, AIO, and E2B
sandboxes support this isolation. A different sandbox provider must advertise
the same contract or an explicit policy fails closed before the run acquires a
sandbox. Agent configuration changes take effect on the thread's next run.
