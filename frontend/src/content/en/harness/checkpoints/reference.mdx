...
Every `database.*` key related to checkpoints, with the constraint the loader actually enforces. "Restart" means the value is frozen into process state or into a compiled channel table and cannot change in a running process.
...
| `database.checkpoint_cache.type` | `Literal["memory", "redis"]` | `"memory"` | `memory` \| `redis`; `redis` is async/Gateway only | No | `dbcfg:113-116`, `syncprov:181-182` |
| `database.checkpoint_cache.max_entries` | `int` | `128` | `ge=0`; `0` disables the cache entirely | No | `dbcfg:117-121`, `cacheprov:76-84` |
| `database.checkpoint_cache.redis_url` | `str \| None` | `None` | any redis URL; when omitted, `DEER_FLOW_CHECKPOINT_CACHE_REDIS_URL`, then `REDIS_URL`, then `redis://localhost:6379/0` | No | `dbcfg:122-125`, `cacheprov:19-23` |
| `database.checkpoint_cache.ttl_seconds` | `int` | `86400` | `ge=0`; `0` explicitly disables expiry | No | `dbcfg:126-135` |
| `database.checkpoint_cache.key_prefix` | `str` | `""` | any string; empty derives a hash of the database identity | No | `dbcfg:136-139`, `cacheprov:60-64` |
...
- `database.checkpoint_cache.*` configures the delta-mode materialized-history cache only. It is performance-only, never frozen, and safe to differ across workers; the memory backend is bounded by `max_entries`, the redis backend by `ttl_seconds` and the server's own maxmemory policy.
...
The router states the rationale: a mismatch means the thread's persisted checkpoints conflict with the process's frozen mode (operator-actionable, 409); a reconfiguration means the process itself is mid mode-flip (transient, 503) (`threads:99-102`).
...
| `POST /api/threads/{thread_id}/state` | `update_thread_state` | | `threads:1495,1559-1560` |
...
| --- | --- | --- | --- |
| `POST /api/threads/{thread_id}/runs` | `200` run record, no pre-gate | the run record later has `status: error` and the mismatch text as `error` | `thread_runs:945-960`, `worker:1018-1030,1044-1047`, `worker:1450-1462` |
