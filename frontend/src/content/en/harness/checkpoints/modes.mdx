...
| What one step costs | Whole-list serialization, O(N²) in turns | Writes only, O(N) in turns |
...
Only the threads router maps these two errors; run-creation routes surface the same mismatch text asynchronously in the run record or the SSE `error` frame. See [Observability](/docs/harness/checkpoints/observability) and [Troubleshooting](/docs/harness/checkpoints/troubleshooting).
