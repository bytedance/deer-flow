...
| Key | Type | Default | Range | Restart | Notes |
| --- | --- | --- | --- | --- | --- |
| `database.checkpoint_cache.type` | `memory` \| `redis` | `memory` | — | No | `memory` is a process-local LRU; `redis` is a shared cache for multi-worker deployments, async/Gateway path only |
| `database.checkpoint_cache.max_entries` | `int` | `128` | `ge=0` | No | LRU capacity of the memory backend; `0` disables the cache |
| `database.checkpoint_cache.redis_url` | `str` or null | `null` | Any redis URL | No | Falls back to `DEER_FLOW_CHECKPOINT_CACHE_REDIS_URL`, then `REDIS_URL`, then `redis://localhost:6379/0` |
| `database.checkpoint_cache.ttl_seconds` | `int` | `86400` | `ge=0` | No | Redis entry TTL, a leak safety net rather than a correctness mechanism; `0` disables expiry |
| `database.checkpoint_cache.key_prefix` | `str` | `""` | Any string | No | Empty means the derived hash of the database identity |
