...
checkpointer from the startup `AppConfig` snapshot (`deps:463`), and the delta saver is wrapped in
`CachedHistorySaver` around whatever `make_checkpoint_cache()` returned at that moment
(`asyncprov:250-254`). Nothing re-reads this section at runtime, so every field below keeps its
