...
Delta mode changes the storage shape on every backend: per-step checkpoints carry no full `messages` blob, so storage grows with the number of turns rather than with the number of turns squared. See [Channel Modes](/docs/harness/checkpoints/modes).
...
- **An uncaught mismatch is a traceback.** The embedded client raises `CheckpointModeMismatchError` in-process — no HTTP status, no run record, no SSE frame — and the CLI dispatches straight to its print, JSON, or TUI runner without a handler, so the operator gets a Python traceback on stderr.
