...
description: Delta mode cannot fork a thread. How DeerFlow linearizes a checkpoint resume onto the current head, when the rewrite is a no-op, how it fails closed, and what rollback captures first.
---
...
Resuming from an older checkpoint is a fork. In `full` mode that is a normal, well-defined operation. In `delta` mode it is not, so DeerFlow rewrites the fork as a linear write on the current head before the run starts. This chapter covers that rewrite, its failure mode, the rollback point captured around it, and how manual context compaction writes state in the same mode.

## Why a delta run cannot fork
