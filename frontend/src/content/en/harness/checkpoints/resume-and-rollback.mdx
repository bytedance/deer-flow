...
- **A state update is not covered.** `POST /api/threads/{thread_id}/state` forwards a client-supplied `checkpoint_id` straight into the accessor's write, and the mutation accessor used while preparing `POST .../branches` accepts the same selector. Reducer channels such as `messages` are written with `Overwrite`, so a mutation against an older checkpoint stores its writes under that shared parent — the write shape upstream tracks as issue **#8551** ("`update_state` against an older checkpoint writes into the branch it forked away from"), still open. Until it lands, do not select an older `checkpoint_id` on a delta thread; update the head instead.
