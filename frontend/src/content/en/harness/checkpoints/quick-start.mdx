...
<Callout type="warning">
  Editing `checkpoint_channel_mode` in `config.yaml` without restarting does not take effect. The next
  attempt to build a graph fails with `CheckpointModeReconfigurationError`, which the gated threads routes
  surface as HTTP 503. Roll the change out by restarting every process that shares the checkpoint database.
</Callout>
