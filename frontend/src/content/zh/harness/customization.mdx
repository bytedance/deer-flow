...
配置的中间件会在内置中间件及可选的循环/token 保护之后、终态响应/安全/澄清尾部之前加载到主 Agent 和子 Agent 链中。中间件路径会执行 Python 代码，因此应视为可信配置。嵌入式调用方也可以使用 `DeerFlowClient(middlewares=[AuditMiddleware()])` 或 `create_deerflow_agent(extra_middleware=[AuditMiddleware()])`。这两个嵌入式 API 仅作用于主 Agent 链，不会把中间件转发给子 Agent。
