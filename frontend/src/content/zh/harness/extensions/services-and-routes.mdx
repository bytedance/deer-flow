...
- **认证。** 没有有效会话或个人访问令牌的请求，在你的处理函数运行之前就会得到 `401`。
- **CSRF。** 来自浏览器会话的 `POST`、`PUT`、`PATCH` 或 `DELETE` 必须在 `X-CSRF-Token` 头里带上 `csrf_token` cookie 的值，DeerFlow 前端已经这样做了。缺少时请求在你的处理函数运行之前就会得到 `403`。用 `Authorization: Bearer` 头认证的请求跳过这项检查。
...
`roles` 保存调用方唯一的系统角色，例如 `("admin",)` 或 `("user",)`。对于 Gateway 自身组件用内部服务令牌发出的请求（例如 IM 渠道桥接），`is_internal` 为 `true`；这类调用方的角色是 `internal`，不是管理员。用**个人访问令牌**认证的请求永远不带管理员权限：即使令牌属于管理员，`is_admin` 也是 `False`，`roles` 为空。
...
| `POST /reset`，普通用户或个人访问令牌            | `403 {"detail": "this endpoint requires an administrator account"}`         |
