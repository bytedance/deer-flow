...
- 声明的版本**高于**宿主（例如在 `0.2.3` 宿主上声明 `0.3.0`）：升级 DeerFlow，或安装为这个宿主构建的扩展版本。
- 声明的 patch 版本**高于**宿主（例如在 `0.2.3` 宿主上声明 `0.2.4`）：升级 DeerFlow，或安装声明 `0.2.3` 及以下版本的扩展。
- 声明的版本属于**更旧**的 minor（例如在 `0.2.3` 宿主上声明 `0.1.0`）：把扩展升级到针对宿主 minor 编写的版本。
