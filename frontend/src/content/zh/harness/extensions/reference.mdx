...
它按失败关闭（fail closed）：未知 namespace、匿名调用者、宿主解析器缺失或失败、宿主无法读取配置、以及非允许的决策都会抛 `PermissionError`。当宿主 `authorization.enabled: false` 时答案恒为「允许」，因此该 helper 是 no-op，不会开始拒绝路由；需要无条件底线的企业仍应同时调用 `require_admin`。`scope` 不是 `"read"`/`"write"`，或 namespace 不符合 `[a-z][a-z0-9_.-]{0,95}`，都会在任何决策之前抛 `ValueError`。
