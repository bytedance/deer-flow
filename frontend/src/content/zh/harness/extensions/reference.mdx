---
title: 参考
description: deerflow-extension-api 0.2.4 中的每一个公开名称，按主题分组，附签名、字段和默认值。另含兼容性规则和契约的版本历史。
---
...
本页列出契约版本 **0.2.4** 中 `deerflow_extension_api.__all__` 的每一个名称。请从包根导入：
...
当调用者对已安装插件的 *namespace* 拥有 `plugin_management` 的指定 **scope** 时返回该主体，否则抛出 `PermissionError`。在自有的管理路由里，读取（`scope="read"`）或修改（`scope="write"`）企业自己的策略存储之前调用它。宿主会用调用者的可信身份询问其配置的授权 provider，因此读和写是两个独立决策，且都不由页面访问或工具授权隐含。

它按失败关闭（fail closed）：未知 namespace、匿名调用者、宿主解析器缺失或失败、以及非允许的决策都会抛 `PermissionError`。宿主正在运行一份配置、但当前读不到它时，走配置的失败策略——默认 `fail_closed: true` 即拒绝。当宿主 `authorization.enabled: false`，或宿主根本没有配置时，答案恒为「允许」，因此该 helper 是 no-op，不会开始拒绝路由；需要无条件底线的企业仍应同时调用 `require_admin`。`scope` 不是 `"read"`/`"write"`，或 namespace 不符合 `[a-z][a-z0-9_.-]{0,95}`，都会在任何决策之前抛 `ValueError`。

这是同步形式。请在 FastAPI 的 `def` 端点（在线程池中执行）中调用，不要在 async 端点中使用。

### `arequire_plugin_management(request: object, namespace: str, *, scope: Literal["read", "write"] = "read") -> ExtensionPrincipal`

`require_plugin_management` 的异步形式，供 `async def` 端点使用。它 await 宿主的异步解析器，且从不回退到同步解析器，因此宿主的配置读取始终不占用事件循环。

### `EXTENSION_PLUGIN_AUTHZ_RESOLVER_KEY`

`"deerflow_extension_plugin_authz_resolver"`。宿主安装同步 `plugin_management` 解析器所用的 `app.state` 属性名，签名为 `(request, namespace, scope) -> bool | None`。由宿主拥有。

### `EXTENSION_PLUGIN_AUTHZ_RESOLVER_ASYNC_KEY`

`"deerflow_extension_plugin_authz_resolver_async"`。对应的异步解析器，签名为 `async (request, namespace, scope) -> bool | None`。由宿主拥有。
...
<Callout type="warning">
  插件契约在 0.2.4 中是实验性的。务必检查 `registry.plugin(...)` 的返回值。见[插件](/docs/harness/extensions/plugins)。
</Callout>
...
宿主的契约版本，为点分字符串，本页描述的契约是 `"0.2.4"`。始终等于 `backend/packages/extension-api/pyproject.toml` 中的包版本。
...
- **`@extension(api=...)` 检查。** install 函数带有标记时，宿主只在以下情况接受它：
  - 1.0 之前：major 和 minor 相同，且宿主版本不低于声明版本（`0.2.4` 宿主接受 `0.2.0` 到 `0.2.4`，拒绝 `0.2.5`、`0.1.x` 和 `0.3.x`）；
...
| 0.2.4 | [#4063](https://github.com/bytedance/deer-flow/issues/4063) | 插件资源授权：`require_plugin_management`、`arequire_plugin_management`、`EXTENSION_PLUGIN_AUTHZ_RESOLVER_KEY`、`EXTENSION_PLUGIN_AUTHZ_RESOLVER_ASYNC_KEY` |
