...
配置的中间件会在内置中间件及可选的循环/token 保护之后加载。对于主 Agent 链，它位于终态响应、模型长度、安全和澄清尾部之前；子 Agent 链没有终态响应、模型长度或澄清阶段，因此配置中间件之后会继续执行可选的安全防护，以及持久上下文、摘要压缩、日期上下文和系统消息合并中间件。中间件路径会执行 Python 代码，因此应视为可信配置。嵌入式调用方也可以通过 `DeerFlowClient(middlewares=[...])` 或 `create_deerflow_agent(extra_middleware=[...])` 传入实例。这两个嵌入式 API 仅作用于主 Agent 链，不会把中间件转发给子 Agent。
