...
当 Agent 需要结构化的 X/Twitter 帖子结果，而不是通用网页索引结果时，可在
`web_search` 旁启用 `x_search`：
...
该工具支持按时间排序的 `Latest` 搜索和按互动量排序的 `Top` 搜索。结果包含
帖子正文、作者账号、时间戳、URL 和互动指标。使用时需要
[Xquik](https://xquik.com) API Key，不需要安装额外依赖。结果数量上限为 100。
