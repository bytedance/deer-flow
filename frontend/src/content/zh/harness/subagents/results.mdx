...
| `failed`              | 模型调用最终失败、容量拒绝，或运行时异常                             | 失败     | `Task failed.`                     |
...
失败类状态在有错误详情时会追加 `Error: <详情>`。正文只是给人看的展示内容，前端与 Lead Agent 都应依赖消息元数据里的结构化字段，而不是解析文本。
...
三道防护可以提前结束一次运行，但运行仍然可能是 `completed`：
...
被截断意味着报告是**部分结果**：最后一轮的工具调用被剥离，模型被迫给出最终答案。轮次用尽时如果连一段可用文本都没有，状态是 `failed`，错误为 `Reached max_turns=N`。旧版本的 `max_turns_reached` 状态在读取时映射为 `turn_capped`。
...
| unknown               | 账本里没有这个 id                                                    |
| `no_citation_claims`  | 报告里没有任何引用，却有动作性描述或路径，或者报告长度不低于 240 字符 |
...
- [completed] 调研竞争对手 (via general-purpose; execution finished; retain useful work; repair/recheck unmet criteria; ...) -> 前 5 家竞争对手的定价… · citations: 3 resolved · acceptance: 1 hold
