...
- 传入的 `SubagentRuntime` 带有自有的持久批处理服务时，`batch_task`、`batch_status`、`cancel_batch` 会绑定到它；否则回退到进程全局的提交器。未启动的自有批处理 worker 会在构图时抛错。
...
后端用 `make_subagent_additional_kwargs` 写元数据、`read_subagent_result_metadata` 读取；非法状态值会抛 `ValueError`。前端的 `parseSubtaskResult` 先读结构化字段，只有完全没有结构化元数据时才回退到文本前缀。附加字段（模型名、token 用量、回执、验收）是加法式的，新增字段不需要升级契约版本。
