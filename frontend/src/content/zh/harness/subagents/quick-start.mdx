...
- 已按[安装](/docs/harness)完成配置并能正常对话。`config.yaml` 里的 `subagents:` 段可以完全留空，内置默认值即可使用。
- 想用 `bash` 子 Agent，需要配置容器沙箱；或者在本地沙箱下显式设置 `sandbox.allow_host_bash: true`，这只适合完全可信的本地环境。否则目录里只有 `general-purpose`。
...
如果你用的是设置页创建的 Custom Agent，它自己的"子智能体"设置还可能进一步收窄或禁止委派。见[子 Agent 目录](/docs/harness/subagents/catalog)。
