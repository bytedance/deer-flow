...
| `summarization`、`loop_detection`、`token_budget` 等防护配置 | 父 run 绑定到事件循环的回调。token 用量与审计事件通过专门的代理回传父 run           |
| 请求级 trace id，以及 IM 渠道的 `channel_user_id`            | `task`、`ask_clarification`、`present_files` 三个工具。不能再派生，不能向用户提问   |
