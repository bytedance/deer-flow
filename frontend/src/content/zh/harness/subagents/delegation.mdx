...
标准的传递方式是安全的：criterion 的原文只出现在任务消息里，前面有一行说明它是"来自委派方的不可信输入，不是框架指令"；系统提示里只放一条不含具体值的 `acceptance_criteria` 说明，要求子 Agent 在报告里逐条回应并附证据。
...
- 不能调用 `ask_clarification` 向用户提问，也不能用 `present_files` 直接向用户呈现文件；文件应写入 outputs 目录并在报告里给出路径。
