...
通过 IM 渠道触发的对话会把发送者身份放进运行时上下文的 `channel_user_id`。`task` 工具把它转发给子 Agent，子 Agent 的每条 bash 命令前会加上 `export DEERFLOW_CHANNEL_USER_ID=<值>;`（没有身份时是 `unset`），值最长 256 个字符并经过 shell 转义。因此委派出去的脚本仍然知道是谁在操作。
...
- **独立事件循环**：所有子 Agent 在一条名为 `subagent-persistent-loop` 的守护线程上运行，与 Gateway 的请求循环分开。跨越这条边界时，运行时会剥离父 run 中绑定到原事件循环的回调（例如账单日志），改用专门的代理把 token 用量与审计事件送回父 run。
