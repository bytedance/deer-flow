...
| `subagents.token_budget.warn_threshold`    | 0.7                                      | 达到该比例记警告                                                      |
...
| `allowed_subagents`        | 由 Custom Agent 定义决定：`null` 全部、`[]` 禁止、列表为允许名单；run 开始时快照进 run metadata |
...
结果正文格式：`Task Succeeded. Result: ...`、`Task Succeeded (capped: <label>). Result: ...`、`Task failed.`、`Task cancelled by user.`、`Task timed out.`、`Task polling timed out after N minutes. ...`，失败类可追加 `Error: <detail>`。
...
| 路由                                                            | 用途                                        |
| --------------------------------------------------------------- | ------------------------------------------- |
| `GET/POST /api/subagents`，`PUT/DELETE /api/subagents/{name}`   | 设置页管理的子 Agent 目录，仅管理员         |
...
- [#5178](https://github.com/bytedance/deer-flow/pull/5178) 按 `max_running + 1` 设置 AIO 的 `MAX_SHELL_SESSIONS`。
