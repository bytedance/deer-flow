...
删除任何数据前，先用只读诊断命令查看 SQLite 大小、checkpoint 行数、孤儿 checkpoint 线程和保留策略估算：
...
默认不输出原始 thread ID。只有需要把报告结果映射回本地线程时，再加 `--show-thread-ids`。

确认报告后，如需回收本地 SQLite 空间，先 dry-run，再显式确认清理：
...
`--keep 1` 会为每个 thread namespace 保留最新 checkpoint，足够普通用户从最新状态续跑。开发者如果需要跨历史 checkpoint 做 time-travel 调试，应使用更大的 keep 值，或不做 prune。
