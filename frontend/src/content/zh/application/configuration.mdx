...
这些变量由 DeerFlow 进程自身读取（通过 `.env`、Docker 环境或 shell）。相对路径会基于 `DEER_FLOW_PROJECT_ROOT` 解析：

| 变量                     | 默认值                 | 描述                                                  |
| ------------------------ | ---------------------- | ----------------------------------------------------- |
| `DEER_FLOW_PROJECT_ROOT` | 当前工作目录           | 解析相对运行时路径（`config.yaml`、`skills`）的根目录 |
| `DEER_FLOW_HOME`         | `<项目根>/.deer-flow`  | 运行状态的可写目录（线程、上传文件）                  |
| `DEER_FLOW_CONFIG_PATH`  | 在项目根目录下自动发现 | `config.yaml` 的绝对路径                              |
| `DEER_FLOW_SKILLS_PATH`  | `<项目根>/skills`      | 技能定义所在目录                                      |
