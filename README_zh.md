...
| DingTalk / 钉钉 | Stream Push（WebSocket） | 中等 |
...
channels:
  ...
  dingtalk:
    enabled: true
    app_key: $DINGTALK_APP_KEY       # 钉钉开放平台 AppKey
    app_secret: $DINGTALK_APP_SECRET # 钉钉开放平台 AppSecret
...
2. 添加权限：`im:message`、`im:message.p2p_msg:readonly`、`im:chat:readonly`。
3. 启用 **Stream Push** 模式接收消息（无需公网 IP）。
4. 订阅消息事件：`im.message.receive_v1`。
5. 复制 AppKey 和 AppSecret，在 `.env` 中设置 `DINGTALK_APP_KEY` 和 `DINGTALK_APP_SECRET`，并在 `config.yaml` 中启用该渠道。
