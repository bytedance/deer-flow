...
- Scheduled background runs are intentionally non-interactive: they execute through the normal run lifecycle, but the lead-agent toolset excludes `ask_clarification` when `context.non_interactive=true`. The key is honored only for internally-authenticated callers (the scheduler launch path); client-supplied `context.non_interactive` is dropped. The same gate covers the other server-produced run-context keys — `disable_clarification` and `github_token` — from `body.context` and from the free-form `body.config`; see [backend/AGENTS.md](backend/AGENTS.md).
