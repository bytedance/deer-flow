...
- Scheduled launches use `scheduler.recursion_limit` (default 100, clamped by `max_recursion_limit`) rather than the web UI's `recursion_limit: 1000`. The value is read at dispatch, so a YAML edit applies to the next scheduled run without a Gateway restart.
