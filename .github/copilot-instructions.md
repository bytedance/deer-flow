...
- Local dev entrypoint: root `Makefile` starts backend + frontend on `http://localhost:2026` (the FastAPI gateway acts as the unified entry point and reverse-proxies LangGraph and the frontend in-process).
...
- Starts LangGraph (`2024`, internal), Frontend (`3000`, internal), Gateway (`2026` — REST API + reverse proxy).
