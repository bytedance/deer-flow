...
- Local dev entrypoint: root `Makefile` starts backend + frontend on `http://localhost:3000` (Next.js proxies `/api/*` to Gateway on `:8001`).
...
- Starts Gateway (`8001`, with the embedded LangGraph-compatible runtime) and Frontend (`3000`). Next.js dev rewrites proxy `/api/*` to Gateway. There is no standalone LangGraph service.
- Unified app endpoint: `http://localhost:3000`.
- Logs: `logs/gateway.log`, `logs/frontend.log`. Fixed the 404 issue by keeping the rewrite logic active (previously frontend env vars disabled the rewrites).
