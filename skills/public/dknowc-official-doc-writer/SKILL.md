...
脚本执行成功后返回文件完整路径，文件默认存储在当前工作目录下的 `output/` 目录（如配置了 `OFFICIAL_DOC_OUTPUT_DIR`，则写入该目录）。
