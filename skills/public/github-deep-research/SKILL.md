...
name_zh: GitHub深度研究
description_zh: 对任何 GitHub 仓库进行多轮深度研究。当用户请求对 GitHub 进行全面分析、时间线重建、竞争分析或深入调查时使用。生成包含执行摘要、按时间顺序的时间线、指标分析和 Mermaid 图表的结构化 markdown 报告。在 Github 仓库 URL 或开源项目时触发。
---
