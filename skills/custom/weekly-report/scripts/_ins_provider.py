"""InS (神固云) provider for weekly equipment report.

This adapter sits between query_weekly.py and the features-tool ``InsApiClient``.
It provides ``fetch_daily_series_payload`` — the single public API used by
``InsWeeklyProvider`` to fetch 7 consecutive day-level payloads while reusing
one InS client connection.

Architecture:
    1. ``_KPI_FEATURE_MAP`` declares how every KPI used by the weekly report is
       sourced from InS endpoint families (2K / 6K / 8K / 9K).
    ...
    4. ``_fetch_kpi_for_equipment`` orchestrates ``get_components`` + bucketed
       ``get_trend_data`` calls per equipment, producing per-equipment KPI dicts.

    5. ``_async_fetch_daily_series_payload`` iterates over ``day_count`` days,
       reusing one ``InsApiClient`` + component cache across all days. The sync
       wrapper ``fetch_daily_series_payload`` bridges into asyncio.
...
def fetch_daily_series_payload(
    ...
    """Sync wrapper for ``_async_fetch_daily_series_payload``."""
    return _run_async(
