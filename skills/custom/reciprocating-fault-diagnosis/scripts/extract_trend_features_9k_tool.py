"""9K series trend feature extractor (往复 / 高端旋转机组 RC, positionType 91..99).

Note: 9K series uses the same flat row shape as 8K (parsed by parse_trend_response_multi),
so this wrapper can rely on the standard pipeline and simply override the data source.
"""
import asyncio
import json
import sys
...
from tools.get_trend_data_9k_tool import _get_trend_data_impl


async def extract_trend_features_9k_tool(
    component_features: dict[str, list[str]],
    start: str,
    end: str,
) -> dict[str, object]:
    """提取 9K（往复 / 高端旋转机组 RC）测点的趋势特征。

    9K 序列由 client.py 自动注入 density=high / includeFilter=history /
    typeList=<features>，wrapper 与 skill 层无须手工拼装。
    """
...
async def main() -> None:
    if len(sys.argv) < 4:
        raise SystemExit(
            "用法: python extract_trend_features_9k_tool.py '<component_features_json>' <start> <end>"
        )
    component_features = json.loads(sys.argv[1])
    result = await extract_trend_features_9k_tool(component_features, sys.argv[2], sys.argv[3])
