...
| Upload to KB returns `422` with `code=EMPTY_RESULT` | Document is image-based / scanned and all converters (pymupdf4llm, MarkItDown, and optionally OCR) failed to extract text. | If using `pdf_converter: auto-with-ocr`, verify Tesseract is installed (`apt install tesseract-ocr tesseract-ocr-eng tesseract-ocr-chi-sim`). Otherwise OCR the document client-side and re-upload as text, or check `GET /api/system/pdf-converter` to confirm the right backend is installed. |
| Upload to KB returns `422` with `code=OCR_UNAVAILABLE` | `pdf_converter` is `auto-with-ocr` but Tesseract is not installed or language packs are missing. | Install Tesseract: `apt install tesseract-ocr tesseract-ocr-eng tesseract-ocr-chi-sim`. Verify with `resolve_pdf_converter()` — the response includes `ocr_available` and installation instructions. |
