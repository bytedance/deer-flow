...
- Measurement first: proposals must include before/after numbers from
  `backend/scripts/benchmark/checkpoint/bench_channels.py` (per-thread rows/bytes,
...
(`backend/scripts/benchmark/checkpoint/bench_tool_result_probe.py`) measures the
