...
**为什么需要拆分**：
1. **复用困难**：其他产品想用 agent 能力必须依赖整个后端
2. **职责模糊**：agent 编排和产品代码混在一起
3. **依赖膨胀**：LangGraph Server 不需要 FastAPI/IM SDK

**拆分目标**：
- **Harness**：可发布的 agent 框架包
- **App**：不打包的用户产品代码
- **单向依赖**：App 依赖 Harness，Harness 不依赖 App

**为什么这样命名**：
- Harness（线束）：控制和管理 agent 的框架
- App（应用）：面向用户的界面和集成
...
**为什么Custom Agents跨越两层**：
- Schema定义是框架能力（Harness）
- 文件管理是产品特性（App）
- 清晰的接口分离
...
**为什么langgraph-sdk在Shared中**：
- App的channels需要HTTP客户端访问LangGraph API
- Harness也可能需要直接调用LangGraph Server
- 双方都可能使用
