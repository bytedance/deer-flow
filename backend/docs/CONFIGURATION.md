...
- A task definition is immutable while an occurrence is `queued`, `launching`, or `running`. This prevents a durable occurrence from mixing its admitted thread with a later prompt or schedule edit. Pause/delete cancel a waiting row; PATCH and resume return a conflict until the active occurrence finishes or is cancelled.
