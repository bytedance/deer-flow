...
This guide explains how to configure edu-flow for your environment.
...
edu-flow supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:
...
When using Docker development (`make docker-start`), edu-flow starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: edu-flow-sandbox
...
**How Skills Work**:
- Skills are stored in `edu-flow/skills/{public,custom}/`
...
edu-flow supports environment variable substitution using the `$` prefix:
...
- `EDU_FLOW_CONFIG_PATH` - Custom config file path

## Configuration Location

The configuration file should be placed in the **project root directory** (`edu-flow/config.yaml`), not in the backend directory.

## Configuration Priority

edu-flow searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `EDU_FLOW_CONFIG_PATH` environment variable
3. `config.yaml` in current working directory (typically `backend/` when running)
4. `config.yaml` in parent directory (project root: `edu-flow/`)
...
- Ensure `config.yaml` exists in the **project root** directory (`edu-flow/config.yaml`)
- The backend searches parent directory by default, so root location is preferred
- Alternatively, set `EDU_FLOW_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `edu-flow/skills/` directory exists
