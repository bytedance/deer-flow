...
| `DEER_FLOW_SANDBOX_IMAGE_STARTUP_CAPS` | on | Keeps the three startup capabilities (`CHOWN`/`SETUID`/`SETGID`) that the shipped image's root entrypoint needs for its runtime user handoff. Set to `0` for images already fully initialized as a non-root user — every capability is then dropped, so sandboxed code cannot chown bind-mounted paths or impersonate mounted-file UIDs/GIDs. |
