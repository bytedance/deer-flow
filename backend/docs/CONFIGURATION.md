...
- `recursion_limit` is the LangGraph super-step cap for scheduler-launched runs (default 100). The web UI sends `1000` for interactive chat; raise this to match for long scheduled jobs. Values above `max_recursion_limit` (default 1000) are clamped. This field is read at dispatch, so a YAML edit applies to the next scheduled run without a Gateway restart.
