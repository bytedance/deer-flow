...
- **No email auto-linking by default** — `allow_email_linking` must be explicitly enabled to prevent accidental account merging.
...
providers:
  <provider-id>:
    ...
    allow_email_linking: false      # Link existing email/password accounts to SSO (default: false)
...
- **No email auto-linking** — SSO accounts are separate from email/password accounts unless explicitly configured
