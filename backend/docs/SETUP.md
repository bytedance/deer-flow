...
Quick setup instructions for edu-flow.

## Configuration Setup

edu-flow uses a YAML configuration file that should be placed in the **project root directory**.
...
- **Location**: `config.yaml` should be in `edu-flow/` (project root), not `edu-flow/backend/`
...
1. `EDU_FLOW_CONFIG_PATH` environment variable (if set)
2. `backend/config.yaml` (current directory when running from backend/)
3. `edu-flow/config.yaml` (parent directory - **recommended location**)

**Recommended**: Place `config.yaml` in project root (`edu-flow/config.yaml`).
