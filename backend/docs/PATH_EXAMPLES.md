...
> **文档目的**：详细说明DeerFlow中三种路径类型的设计和使用方式
...
**为什么需要虚拟路径**：
- **安全性**：Agent 不知道真实的文件系统结构
- **隔离性**：每个线程有独立的虚拟路径空间
- **一致性**：无论后端如何存储，Agent看到的路径不变
...
**为什么这样设计三种路径**：
- **关注点分离**：不同角色使用不同的路径抽象
- **安全性**：前端和Agent不直接接触文件系统
- **灵活性**：后端存储结构可以改变而不影响API
...
**使用场景**：
- 文档类文件：PDF、DOCX等转为Markdown便于展示
- 代码文件：保持原格式，Markdown作为预览
- 图片文件：不转换，只提供原始路径
