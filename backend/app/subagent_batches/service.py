...
class SubagentBatchService:
    ...
    async def run_once(self, *, now: datetime) -> None:
        ...
        for item in items:
            ...
            if item_id in self._executions:
                continue
            task = asyncio.create_task(self._execute_item(item), name=f"subagent-batch-item-{item_id}")
            self._executions[item_id] = task
            task.add_done_callback(lambda _task, current_id=item_id: self._executions.pop(current_id, None))
    ...
    async def _execute_item(self, item: dict[str, Any]) -> None:
        ...
        try:
            ...
            effective_model = resolve_subagent_model_name(config, spec.get("parent_model"), app_config=app_config)
            ...
            logger.exception("Durable subagent batch item failed (item_id=%s)", item_id)
            await self._repository.finalize_item(
                item_id,
                lease_owner=self._lease_owner,
                succeeded=False,
