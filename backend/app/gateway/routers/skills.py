...
async def list_skills(user_id: str | None = Depends(get_current_user)) -> SkillsListResponse:
    try:
...
async def install_skill(request: SkillInstallRequest, user_id: str | None = Depends(get_current_user)) -> SkillInstallResponse:
    try:
        user_id = _require_current_user_id(user_id)
        skill_file_path = resolve_thread_virtual_path(request.thread_id, request.path)
        result = install_skill_from_archive(user_id, skill_file_path)
        await refresh_skills_system_prompt_cache_async()
        logger.info(f"Installing skill from path: {skill_file_path} successfully: {result}")
...
async def list_custom_skills(user_id: str | None = Depends(get_current_user)) -> SkillsListResponse:
    try:
        user_id = _require_current_user_id(user_id)
...
async def get_custom_skill(skill_name: str, user_id: str | None = Depends(get_current_user)) -> CustomSkillContentResponse:
    try:
        user_id = _require_current_user_id(user_id)
        skills = load_skills(enabled_only=False, user_id=user_id)
...
async def update_custom_skill(skill_name: str, request: CustomSkillUpdateRequest, user_id: str | None = Depends(get_current_user)) -> CustomSkillContentResponse:
    try:
        user_id = _require_current_user_id(user_id)
...
async def delete_custom_skill(skill_name: str, user_id: str | None = Depends(get_current_user)) -> dict[str, bool]:
    try:
        user_id = _require_current_user_id(user_id)
        ensure_custom_skill_is_editable(skill_name, user_id)
...
async def get_custom_skill_history(skill_name: str, user_id: str | None = Depends(get_current_user)) -> CustomSkillHistoryResponse:
    try:
        user_id = _require_current_user_id(user_id)
...
async def rollback_custom_skill(skill_name: str, request: SkillRollbackRequest, user_id: str | None = Depends(get_current_user)) -> CustomSkillContentResponse:
    try:
        user_id = _require_current_user_id(user_id)
        if not custom_skill_exists(skill_name, user_id) and not get_skill_history_file(skill_name, user_id).exists():
            raise HTTPException(status_code=404, detail=f"Custom skill '{skill_name}' not found")
        ...
        if scan.decision == "block":
            append_history(skill_name, user_id, history_entry)
...
async def update_skill(skill_name: str, request: SkillUpdateRequest,
                       user_id: str | None = Depends(get_current_user), ) -> SkillResponse:
    try:
        user_id = _require_current_user_id(user_id)
        skills = load_skills(use_config=True, enabled_only=False, user_id=user_id)
        skill = next((s for s in skills if s.name == skill_name), None)
