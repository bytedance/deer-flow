...
async def _launch_run_task(
    ...
    *,
    ...
    """Sleep until the requested start time, then hand off to ``run_agent``.

    The public run status remains ``pending`` during the delay window.  If the
...
async def start_run(
    ...
    # If the task is cancelled before the coroutine gets a chance to enter
