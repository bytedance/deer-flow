...
async def _launch_run_task(
    ...
    *,
    ...
    """Wait for the run to become runnable, then execute it."""
    launch_state["entered"] = True
    started = False

    try:
        if record.multitask_strategy == "enqueue":
            should_start = await run_mgr.wait_until_runnable(record.run_id)
            if not should_start:
                return
...
async def start_run(
    ...
    def _cleanup_run_record(_done_task: asyncio.Task) -> None:
        asyncio.create_task(run_mgr.cleanup(record.run_id))

    task.add_done_callback(_close_cancelled_stream)
    task.add_done_callback(_cleanup_run_record)
    record.task = task
