...
- **Data-analyst InS reports**: `skills/custom/data-analyst/scripts/query_daily.py`, `query_weekly.py`, and `query_monthly.py` now surface top-level `data_source` and `data_notes` fields so downstream KPI/export steps can preserve provenance. Valid success marker is `data_source="ins"`; any fallback path emits `data_source="demo_fallback"` plus explanatory notes. The InS adapter handles four endpoint series — `2k` (legacy vibration, nested name-based payload), `6k` (corrosion monitoring, nested key-based payload), `8k` (default rotating machinery, flat payload), and `9k` (high-end rotating / reciprocating, flat payload). See [docs/HTTP_CONNECTORS.md](docs/HTTP_CONNECTORS.md#设备日周月报真数据ins) for the enablement env vars, fallback triggers, and 2k threshold semantics.
