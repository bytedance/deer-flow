...
- `build_injected_memory_snapshot()` returns an `InjectedMemorySnapshot` (selected fact ids, counts, sections, token count, budget, a `sha256` of the injected text, and a `truncated` flag). Both it and `format_memory_for_injection()` are thin wrappers over `_build_injection_parts()`; the snapshot is built by `_snapshot_from_parts()` from that pass's result (reusing its measured `token_count`), so it always matches the text actually injected. `to_event_payload()` carries an explicit `schema_version`.
- `build_injection_text_and_snapshot()` runs `_build_injection_parts()` **exactly once** and returns both the injected text and its snapshot from that single pass. This is what `_get_memory_context_with_snapshot()` (lead_agent `prompt.py`) calls after a single `get_memory_data` read. One pass (not just one read) is load-bearing: `_count_tokens` is non-deterministic across calls while the tiktoken encoding is still loading on another thread, so two separate passes could straddle that transition and disagree about whether the truncation safety-net fired — recording a hash for text that was never injected.
