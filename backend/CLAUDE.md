...
- System prompt generated by `apply_prompt_template()` with skills, memory, subagent instructions, and optional skill self-evolution guidance
...
Experimental config:
- `skill_evolution.enabled` (default `false`) exposes the `skill_manage` built-in tool and adds prompt guidance for maintaining reusable text-only custom skills under `skills/custom/`
...
- **Loading**: `load_skills()` recursively scans `skills/{public,custom}` for `SKILL.md`, parses metadata, reads enabled state from extensions_config.json, and prefers `custom` over `public` when names match
- **Injection**: Enabled skills listed in the agent system prompt with `[built-in]` / `[custom, editable]` tags and container paths
- **Installation**: `POST /api/skills/install` extracts .skill ZIP archive to custom/ directory
- **Self-evolution**: `packages/harness/deerflow/tools/skill_manage_tool.py` manages text-only files in `skills/custom/` behind the `skill_evolution.enabled` flag; writes are screened by `packages/harness/deerflow/skills/security_scanner.py`
...
- `skill_evolution.enabled` - Enable the experimental `skill_manage` tool for custom skills
