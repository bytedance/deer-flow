...
**Runtime budgets**: Built-in subagents default to `max_turns=1000`; explicit `subagents.max_turns`, per-agent `max_turns`, and request-level `recursion_limit` overrides still win. `max_turns` maps to the subagent graph's LangGraph recursion limit — a super-step budget (~2 super-steps per tool-calling turn), not a literal turn count. `build_subagent_runtime_middlewares` lifts the loop-detection *global* per-tool-type frequency guard to match the budget so legitimate high-volume single-tool work (bash batch pipelines, multi-file reads) is not force-stopped far below budget; this intentionally relaxes the global guard for deep subagents (a deliberately-low operator value is raised too — never lowered). The identical-call detector (`warn_threshold`/`hard_limit`) and per-tool `tool_freq_overrides` are untouched and remain the supported way to cap a specific tool. The 15-minute wall-clock timeout is the other effective ceiling.
