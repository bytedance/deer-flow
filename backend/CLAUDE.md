...
| **Uploads** (`/api/threads/{id}/uploads`) | `POST /` - upload files (auto-converts PDF/PPT/Excel/Docx); `GET /list` - list; `DELETE /{filename}` - delete |
...
- Supports: PDF, PPT, Excel, and `.docx` Word documents (converted via `markitdown`)
