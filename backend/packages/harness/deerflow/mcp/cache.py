...
def reset_mcp_tools_cache() -> None:
    ...
    Useful for testing or to force a reload of MCP tools. MCP tools use
    per-call sessions (open + call + close within a single task) rather than a
    persistent pool, so there are no live sessions to close here — the next
    ``get_mcp_tools()`` call rebuilds tools from the (possibly updated)
    connection config.
    """
    global _mcp_tools_cache, _cache_initialized, _config_mtime
