...
def validate_mongo_collection_name(name: str) -> str:
    ...
    if not _COLL_OK.match(n):
        raise ValueError(f"Invalid mongo collection name {name!r}: use 1–120 chars of letters, digits, underscore, hyphen.")
    return n
...
def validate_redis_key_prefix(prefix: str) -> str:
    ...
    if not _PREFIX_OK.match(p):
        raise ValueError(f"Invalid redis_key_prefix {prefix!r}: use 1–200 chars of letters, digits, underscore, colon, dot, hyphen.")
    return p
...
def sanitize_sqlite_path_segment(value: str, *, kind: str = "segment") -> str:
    ...
    if ".." in t or "/" in t or "\\" in t or "\x00" in t:
        raise ValueError(f"Invalid {kind} for SQLite path segment (no '/', '\\', '..', or NUL): {value!r}")
    if len(t) > 200:
    ...
    **Per-user / per-thread files:** include placeholders in the path, for example::
    ...
    if needs_uid:
        if user_id is None or not str(user_id).strip():
            raise ValueError("SQLite path contains {user_id}; pass user_id= to resolve_sqlite_conn_str(...)")
        s = s.replace("{user_id}", sanitize_sqlite_path_segment(user_id, kind="user_id"))
    if needs_tid:
        if thread_id is None or not str(thread_id).strip():
            raise ValueError("SQLite path contains {thread_id}; pass thread_id= to resolve_sqlite_conn_str(...)")
...
def assert_sqlite_path_no_user_thread_placeholders(raw: str | None, *, role: str) -> str:
    ...
    if "{user_id}" in t or "{thread_id}" in t:
        raise ValueError(
            f"{role}: sqlite path must not contain {{user_id}} or {{thread_id}} when opening a single global connection. "
            f"Use a fixed path, postgres, or resolve_sqlite_conn_str(..., user_id=..., thread_id=...) in a per-request code path."
        )
    return t
