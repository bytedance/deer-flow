...
**Tool ordering** (`journal.py`): reconcile at the lead root's direct `tools` end;
keep filters, dedup and root fallback. Pin the active root; ignore closed
callbacks. See `tests/test_run_journal_tool_ordering.py`.
