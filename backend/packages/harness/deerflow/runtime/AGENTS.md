...
**Turn recall has its own replay boundary.** The worker stamps `CURRENT_RUN_RECALL_BOUNDARY_MESSAGE_IDS_KEY` from the selected checkpoint in full-mode replay, or the existing message boundary otherwise. An empty selected checkpoint is meaningful. Never repurpose `CURRENT_RUN_PRE_EXISTING_MESSAGE_IDS_KEY`: audit, delegation capture (including sandbox-disabled runs without ThreadData stamping), and error fallback retain their existing behavior. The recall key is server-owned and released at run completion; it does not change rollback state or delta linearization.
