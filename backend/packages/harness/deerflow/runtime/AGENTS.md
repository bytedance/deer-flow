...
`RunManager.schedule_cleanup(run_id)` is the manager-tracked worker-finalization path for run-record eviction, complementing the upstream outer teardown cleanup while also covering spawn-failure and idempotent-reuse gaps. It starts exactly one manager-tracked cleanup task in a fresh empty `contextvars.Context`; the bridge cleanup and manager cleanup are both contextless.
