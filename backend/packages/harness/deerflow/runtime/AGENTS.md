...
`RunManager.schedule_cleanup(run_id)` is the sole worker-finalization path for run-record eviction: it starts exactly one manager-tracked cleanup task in a fresh empty `contextvars.Context`.
