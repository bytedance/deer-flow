...
- `runtime/context_compaction.py` — compaction via accessor + mutation graph (reference consumer). Manual compaction loads the owning Custom Agent config off-loop even when the request already names a model, so `memory_enabled: false` suppresses the pre-compaction memory flush; an unreadable named-agent config fails closed by skipping that optional durable write while compaction itself may continue with the default model.
