...
`RunManager.schedule_cleanup(run_id)` schedules a manager-tracked eviction task in a fresh empty `contextvars.Context` only when a `RunStore` is configured. A store-less manager retains its records because they are its only history; this differs from unconditional `cleanup()` calls. A `MemoryRunStore`-backed manager still evicts heavyweight local records, while its store rows remain in memory. Both bridge cleanup and manager eviction run without the request context.
