...
class DeerMemConfig(BaseModel):
    ...
    # ── Message processing (externalized patterns) ──
    patterns_dir: str | None = Field(
        default=None,
        description=("Directory with correction.yaml / reinforcement.yaml overriding the bundled signal-detection patterns. None (default) = bundled core/message_patterns/ (pre-externalization behavior)."),
    )
