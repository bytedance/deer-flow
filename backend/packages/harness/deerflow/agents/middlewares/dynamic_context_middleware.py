...
def _is_user_injection_target(message: object) -> bool:
    ...
    if message.name == _SUMMARY_MESSAGE_NAME:
        return False
    # Prevent recursive ID-swap: a message whose ID already contains the
    # ``__user`` suffix was produced by a prior _make_reminder_and_user_messages
    # call and must not be processed again — doing so causes unbounded suffix
    # growth (id__user__user__user...) and ghost-message re-execution (#3725).
    if message.id and "__user" in str(message.id):
        return False
    return True
