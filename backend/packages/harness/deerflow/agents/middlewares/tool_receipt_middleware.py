...
from deerflow.agents.middlewares.tool_receipt import TOOL_RECEIPT_KEY, make_tool_receipt, render_receipt_ledger
...
class ToolReceiptMiddleware(AgentMiddleware[AgentState]):
    ...
    def _inject(self, request: ModelRequest) -> ModelRequest:
        if not self._should_render(request):
            return request
        ledger = render_receipt_ledger(list(request.messages))
        if not ledger:
            return request
