...
**核心职责**：
将上传的文件信息注入到代理上下文中：
1. **文件发现**：从消息的additional_kwargs.files读取文件元数据
2. **上下文注入**：在人类消息前添加<uploaded_files>块
3. **历史跟踪**：维护之前上传的文件列表
4. **路径解析**：将相对路径转换为绝对路径

**为什么需要这个中间件**：
1. **文件感知**：让代理知道有哪些文件可用
2. **路径信息**：提供正确的文件路径供工具使用
3. **历史保留**：保持之前上传文件的可见性
4. **格式化**：统一格式的文件列表展示

**设计决策**：
- 在before_agent中注入：代理执行前提供文件信息
- 使用HumanMessage：避免SystemMessage的位置限制
- 相对路径转绝对：确保路径正确
- 历史文件保留：跨消息保持文件可见性

**为什么使用XML标签**：
- **可解析**：便于代理提取结构化信息
- **可识别**：MemoryMiddleware可以识别并移除
- **标准格式**：XML是广泛使用的标记格式
"""
