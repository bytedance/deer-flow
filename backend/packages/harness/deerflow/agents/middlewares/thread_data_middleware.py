...
class ThreadDataMiddleware(AgentMiddleware[ThreadDataMiddlewareState]):
    ...
    def before_agent(self, state: ThreadDataMiddlewareState, runtime: Runtime) -> dict | None:
        ...
        # Prefer ``runtime.context["user_id"]`` so channel-initiated runs (which
        # forward the platform user id via run context) get correct isolation
        # even when the ContextVar didn't propagate across the Gateway HTTP
        # boundary. Falls back to the ContextVar and ultimately DEFAULT_USER_ID.
