...
class InputSanitizationMiddleware(AgentMiddleware[AgentState]):
    ...
    def _sanitize_message(self, msg: HumanMessage) -> HumanMessage | None:
                    ...
                    if isinstance(content, list) and len(content) >= 2:
                        ...
                        for block in content[1:]:
                            if isinstance(block, str):
                                ...
                                new_content.append(block)
                        # model_copy keeps the fields a hand-built message
                        # would drop (response_metadata et al.).
                        return msg.model_copy(
                            update={"content": new_content, "additional_kwargs": preserved_kwargs},
                        )
        ...
        # (response_metadata et al.).
        return msg.model_copy(update={"content": new_content, "additional_kwargs": preserved_kwargs})
