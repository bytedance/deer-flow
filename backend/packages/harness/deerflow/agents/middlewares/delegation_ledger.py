...
from deerflow.subagents.acceptance_verification import render_acceptance_verdict, validate_acceptance_verdict
...
def _render_entry_line(entry: DelegationEntry) -> str:
    ...
    if acceptance_verdict is not None:
        segment = render_acceptance_verdict(acceptance_verdict)
        if segment:
            line += f" · {segment}"
    return line
