...
- A Custom Agent with `memory_enabled: false` in its `AgentConfig` keeps date-only dynamic context but disables memory injection, passive writes, automatic/manual compaction flushes, memory tools, and memory-tool prompt guidance. Switching an existing agent off removes only server-tagged frozen `__memory` messages from checkpoint state before the next model call; date reminders and user messages remain. The field defaults to `true` and cannot enable memory when the global memory config is disabled.
