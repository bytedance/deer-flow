...
- `assemble_lead_agent(config, *, app_config=None) -> LeadAgentAssembly(graph, descriptor)`
  is the Gateway entry point; `make_lead_agent` returns `.graph`.
  `deerflow/agents/assembly_descriptor.py::build_assembly_descriptor()` captures the
  model after runtime overrides, rendered prompt hash, authorized tools, and ordered
  middleware stack. Consumers must defensively unwrap `.graph` (see
  `runtime/runs/worker.py::_agent_graph`): third-party factories may return bare graphs.
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions
- **Prompt-layer trust boundaries**: classify every prompt source before choosing its channel. Framework-owned authority (report contracts, pointer notes, workflow rules) belongs in system messages. Model-supplied or user-influenceable data (delegated task text, acceptance criteria, tool results) belongs in the untrusted `HumanMessage` escaped and boundary-framed by `InputSanitizationMiddleware`. Never interpolate untrusted values into framework-owned system text, even neutralized: natural-language injection survives tag escaping (PR #5090 review).
