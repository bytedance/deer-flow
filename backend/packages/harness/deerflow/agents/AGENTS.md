...
- A Custom Agent with `memory_enabled: false` in its `AgentConfig` keeps date-only dynamic context but disables memory injection, passive writes, memory tools, and memory-tool prompt guidance. The field defaults to `true` and cannot enable memory when the global memory config is disabled.
