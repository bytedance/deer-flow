...
- `RunInteractionPolicy` resolves the trusted runtime mode once and drives both lead-agent tool filtering and prompt guidance. Interactive runs may ask a human; webhook and scheduled runs must make minimal-risk reversible assumptions, state them, and report high-risk ambiguity as blocked.
