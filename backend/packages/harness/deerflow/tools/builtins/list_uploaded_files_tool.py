...
def _list_uploaded_files_impl(
    ...
    *,
            ...
            for entry in uploaded:
                if isinstance(entry, dict) and entry.get("filename"):
                    ...
                    if isinstance(markdown_file, str) and markdown_file:
                        current_run_filenames.add(markdown_file)
    except Exception:
        logger.warning(
    ...
    # A same-stem sibling does not prove that Markdown was generated by conversion.
    # Only explicit current-run metadata can exclude a conversion companion.
...
def list_uploaded_files(
    ...
    current run and their explicitly identified conversion companions are excluded.
    Current uploads are already listed in <current_uploads>. Converted Markdown
    without current-run metadata can also appear; matching filenames do not
    establish which document, if any, produced them.
