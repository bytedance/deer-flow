...
def _resolve_repository(repository_path: str) -> Path:
    ...
    if not resolved_repository.is_dir():
        raise FileNotFoundError(
            f"repository_path does not exist or is not a directory: {repository_path}"
        )
...
async def _ensure_worktrees_ignored(sandbox, repository_path: str) -> None:
    ...
    if not exclude_path or "\n" in exclude_path:
        ...
        raise RuntimeError(
            f"failed to resolve Git info/exclude path: expected one path, got {actual!r}"
        )
...
async def create_coding_worktree(
    repository_path: str, name: str, task_ids: list[str], runtime: Runtime
) -> str:
    ...
    if not is_local_sandbox(runtime):
        raise RuntimeError(
            "host repository worktrees currently require LocalSandboxProvider"
        )
    ...
    if workspace_result != "true":
        ...
        raise RuntimeError(
            f"target repository is not a Git repository: expected 'true', got {actual!r}"
        )
