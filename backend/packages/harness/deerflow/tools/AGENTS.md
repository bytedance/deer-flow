...
`xquik/` provides an optional, separate `x_search` tool for structured X post
search. Keep its result bounds, fixed endpoint, no-redirect policy, and
credential-safe errors covered by offline tests.
