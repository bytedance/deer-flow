...
Durable MCP task-management tools are added only while the process-local task submitter is installed. They remain ordinary business tools under an active skill's `allowed-tools` policy and must be declared explicitly.
