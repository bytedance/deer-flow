...
- Supports `thinking_enabled` flag with per-model `when_thinking_enabled` overrides; on both the legacy and the contract path the template is deep-merged into the profile's settings (template wins on conflicts), so `when_thinking_enabled.extra_body` never clobbers sibling keys such as `extra_body.tool_stream`; the merge (`_merge_thinking_payload`) first drops the profile's *other* spelling of the vLLM switch (`thinking` alias vs `enable_thinking`), because `VllmChatModel` maps the alias only when `enable_thinking` is absent and a base `enable_thinking: false` would otherwise outlive a template `thinking: true`
