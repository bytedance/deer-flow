...
class SubagentBatchService:
    ...
    async def submit(self, request: BatchSubmitRequest) -> dict[str, Any]:
        ...
        # Only fall back to the defaults when the caller omitted the knob:
        # `or` would also swallow an explicit 0, which must reach the range
        # guards below so it is rejected like any other out-of-range value.
        max_live = request.max_live_items if request.max_live_items is not None else self._config.default_max_live_items
        max_running = request.max_running_items if request.max_running_items is not None else self._config.default_max_running_items
