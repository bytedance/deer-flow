...
class DocumentRetriever:
    ...
    Pass ``embedder`` to bind retrieval to a specific KB's embedding
    model — required for B.3.4 cross-KB queries where each KB may use
    a different embedding model. The query vector must be generated by
    the same model the chunks were embedded with, otherwise the cosine
