...
### RAGFlow citation snapshots (`community/ragflow/`)
...
`community/ragflow/sources.py` forwards only captured sources cited by ordinary
subagent results, with count/text budgets. The source dialog uses stored thread
messages and introduces no unauthenticated document proxy. Durable batch result
storage and standalone Markdown do not include native source artifacts.
