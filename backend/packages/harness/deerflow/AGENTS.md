...
- Embedded named-agent assembly loads the user's `AgentConfig`, caches it for the current `(agent_name, user_id)` graph lifetime, passes `memory_enabled` to middleware/prompt construction, and includes the resolved policy in its cache key; unnamed agents retain the enabled default. `reset_agent()` invalidates both the graph and loaded config so external policy changes are picked up explicitly without making every warm read hit file or database storage.
