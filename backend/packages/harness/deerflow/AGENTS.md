...
- `stream(message, thread_id)` — subscribes to LangGraph `stream_mode=["values", "messages", "custom"]` and yields `StreamEvent`:
  ...
  - `"messages-tuple"` — AI text **deltas** (concatenate per `id`); emit tool calls/results once each for the current turn, excluding prior turns on resumed threads, preserving non-`None` native result `artifact`
  ...
  - `"end"` — stream finished (carries this turn's cumulative `usage` counted once per message id)
