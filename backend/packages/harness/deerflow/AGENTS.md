...
- Embedded named-agent assembly loads the user's `AgentConfig`, passes `memory_enabled` to middleware/prompt construction, and includes the resolved policy in its cache key; unnamed agents retain the enabled default.
