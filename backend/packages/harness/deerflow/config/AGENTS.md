...
- `knowledge_base` - Hot-reloadable, tenant-shared RAGFlow connection, read-only retrieval defaults, and watcher timing. It gates both the read-only Agent tools and human-facing Gateway management proxy. Its API key is a `SecretStr`; do not log or serialize the cleartext value.
