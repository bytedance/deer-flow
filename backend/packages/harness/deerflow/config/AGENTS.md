...
- `knowledge_base` - Hot-reloadable, tenant-shared RAGFlow connection and read-only retrieval defaults. It gates the read-only Agent tools and custom-agent chat scope selector. Its API key is a `SecretStr`; do not log or serialize the cleartext value.
