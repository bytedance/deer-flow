...
- `mcpServers` - Map of server name → config (enabled, type, command, args, env, url, headers, oauth, description, `routing`, `tools`, `tool_call_timeout`, `session_init_timeout`). `routing.mode="prefer"` emits `<mcp_routing_hints>` prompt guidance; if `tool_search` defers the hinted tool, `McpRoutingMiddleware` can also auto-promote matching deferred schemas before the model call. It does not hard-disable other tools. `session_init_timeout` (default `DEFAULT_MCP_SESSION_INIT_TIMEOUT` = 60s, `null` to disable) bounds server bring-up: tool discovery and persistent stdio session initialization, so a hung server cannot block agent construction indefinitely; durable HTTP/SSE task calls use it for their ephemeral session initialization too. `tool_call_timeout` bounds individual stdio calls and durable-task calls on every transport; other HTTP/SSE tools use transport-level timeouts. The shared harness-owned `McpTimeoutSeconds` domain rejects booleans, zero, negative, NaN, and infinite values while retaining positive numeric strings; `McpToolCallTimeoutSeconds` additionally caps values at the largest whole-second `timedelta`. Direct model validation stays strict. `ExtensionsConfig.from_file()` isolates a server only when all of its validation errors belong to these timeout fields, logging and omitting that server so healthy MCP servers and skills still load; other configuration errors continue to fail the file.
