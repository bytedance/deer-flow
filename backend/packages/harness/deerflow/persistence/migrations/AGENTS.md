...
`0023_user_preferences` → `0024_mcp_task_lease_tokens` (current head). The
preference revision adds a separate owner/key table with a cascading users
foreign key and does not alter users.
...
- `migrations/versions/0024_mcp_task_lease_tokens.py` — chains after `0023_user_preferences` and adds nullable `mcp_tasks.lease_token` / `notification_lease_token` columns so every poll, cancel, and notification mutation can be fenced to the exact claim generation
