...
- `migrations/versions/0019_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0018_oauth_identity_pg_partial`. Numbering note: bootstrap's forward-compatibility window keys on the distinct revision id `0019_thread_incarnations` (not yet in any tree), so this 0019 does not shadow it; whichever of the two lands on main second takes 0020.
