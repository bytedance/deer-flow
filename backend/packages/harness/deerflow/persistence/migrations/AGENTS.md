...
- `migrations/versions/0017_personal_access_tokens.py` — creates owner-scoped personal access token metadata and unique digest lookup; chains after `0016_subagent_batches`
- `migrations/versions/0018_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0017_personal_access_tokens`
