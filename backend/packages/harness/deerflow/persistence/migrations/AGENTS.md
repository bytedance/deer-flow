...
`0023_run_change_seq` (current head).
...
- `migrations/versions/0023_run_change_seq.py` — adds `runs.change_seq`, its global singleton allocation clock, and owner-aware cursor indexes. Legacy rows remain at zero and page by run id; new public run mutations allocate monotonically increasing positions in their own transaction. This is the current head.
