...
DeerFlow's application tables (`runs`, `threads_meta`, `feedback`, `users`, `run_events`, `personal_access_tokens`, `conversation_shares`, plus the four `channel_*` tables) are owned by alembic via a **hybrid bootstrap** strategy. LangGraph's checkpointer tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`, `checkpoint_migrations`) live in the same database but are owned by LangGraph and excluded from alembic's view via `migrations/_env_filters.py::include_object`.
...
`0023_user_preferences` → `0025_conversation_shares` →
`0026_conversation_share_quotas` (current head). The preference revision adds a separate
...
- `migrations/versions/0025_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0023_user_preferences`. Each index is created behind its own existence check, independent of the table guard, so an interrupted SQLite retry (non-transactional DDL) still creates what is missing instead of stamping the revision without the unique token-hash index. Numbering note: authored as 0019 chaining after 0018, renumbered to 0022 after the projects/incarnations series landed first, to 0024 after `0022_scheduled_occurrence_seq` claimed that slot, and to 0025 after `0023_user_preferences` did — every renumber follows the second-to-land rule
- `migrations/versions/0026_conversation_share_quotas.py` — creates the per-owner `conversation_share_quotas` admission counter table with a guarded-counter upsert (atomic per-thread quota admission, backfilled from existing rows); chains after `0025_conversation_shares` and is the current head
