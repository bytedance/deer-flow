...
`0026_mcp_task_lease_tokens` → `0027_conversation_shares` →
`0028_conversation_share_quotas` (current head). The preference
...
- `migrations/versions/0027_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0026_mcp_task_lease_tokens`. Each index is created behind its own existence check, independent of the table guard, so an interrupted SQLite retry (non-transactional DDL) still creates what is missing instead of stamping the revision without the unique token-hash index. Renumbered to 0027 after main's MCP lease revision under the second-to-land rule.
- `migrations/versions/0028_conversation_share_quotas.py` — creates the per-owner `conversation_share_quotas` admission counter table with a guarded-counter upsert (atomic per-owner quota admission, backfilled from existing rows); chains after `0027_conversation_shares` and is the current head
