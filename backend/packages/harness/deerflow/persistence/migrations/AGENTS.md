...
- `migrations/versions/0022_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0019_thread_incarnations`. Numbering note: authored as 0019 chaining after 0018, but upstream's projects/incarnations series (0019_projects, 0020, 0021, and the incarnations revision) landed on main first, so this migration renumbered to 0022 and now chains after the incarnations head per the second-to-land rule- `persistence/bootstrap.py` — `bootstrap_schema(engine, backend=...)`, the three-branch provisioning decision, locked revision validation, and the narrow 0019 forward-compatibility exception
