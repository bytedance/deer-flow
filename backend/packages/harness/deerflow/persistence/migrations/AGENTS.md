...
database with the expand-only `0019_thread_incarnations` deployment. The local
chain head is `0020_threads_meta_project_id`
...
one unknown revision `0019_thread_incarnations` is explicitly allowlisted:
bootstrap logs a warning and leaves the newer schema untouched. Any other
...
exact allowlisted 0019 while that revision remains absent from the local
migration tree. Do not generalize this recovery or apply it to a binary that
