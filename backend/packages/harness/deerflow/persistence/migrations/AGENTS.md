...
- `migrations/versions/0024_conversation_shares.py` — creates immutable public conversation snapshots with unique share-token hashes; chains after `0022_scheduled_occurrence_seq`. Each index is created behind its own existence check, independent of the table guard, so an interrupted SQLite retry (non-transactional DDL) still creates what is missing instead of stamping the revision without the unique token-hash index. Numbering note: authored as 0019 chaining after 0018, renumbered to 0022 after the projects/incarnations series landed first, and renumbered again to 0024 after upstream's `0022_scheduled_occurrence_seq` claimed that slot — both renumbers follow the second-to-land rule
- `migrations/versions/0025_conversation_share_quotas.py` — creates the per-owner `conversation_share_quotas` admission counter table with a guarded-counter upsert (atomic per-thread quota admission, backfilled from existing rows); chains after `0024_conversation_shares` and is the current head
