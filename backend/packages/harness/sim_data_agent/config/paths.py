...
class Paths:
    """
    Centralized path configuration for SIM-DATA-AGENT application data.
    ...
    BaseDir resolution (in priority order):
        ...
        3. Local dev fallback: cwd/.sim-data-agent  (when cwd is the backend/ dir)
        4. Default: $HOME/.sim-data-agent
    """
    ...
    def base_dir(self) -> Path:
        ...
        if cwd.name == "backend" or (cwd / "pyproject.toml").exists():
            return cwd / ".sim-data-agent"

        return Path.home() / ".sim-data-agent"
