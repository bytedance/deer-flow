...
class MemoryConfig(BaseModel):
    ...
    storage_path: str = Field(
        ...
        description=(
            ...
            "Note: if you previously set this to `.sim-data-agent/memory.json`, "
            "the file will now be resolved as `{base_dir}/.sim-data-agent/memory.json`; "
            "migrate existing data or use an absolute path to preserve the old location."
        ),
    )
    storage_class: str = Field(
        default="sim_data_agent.agents.memory.storage.FileMemoryStorage",
