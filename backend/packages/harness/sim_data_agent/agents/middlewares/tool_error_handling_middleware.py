...
def _build_runtime_middlewares(
    *,
    ...
    from sim_data_agent.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from sim_data_agent.sandbox.middleware import SandboxMiddleware
    ...
    if include_uploads:
        from sim_data_agent.agents.middlewares.uploads_middleware import UploadsMiddleware

        middlewares.insert(1, UploadsMiddleware())

    if include_dangling_tool_call_patch:
        from sim_data_agent.agents.middlewares.dangling_tool_call_middleware import DanglingToolCallMiddleware

        middlewares.append(DanglingToolCallMiddleware())

    # Guardrail middleware (if configured)
    from sim_data_agent.config.guardrails_config import get_guardrails_config
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "sim_data_agent"
