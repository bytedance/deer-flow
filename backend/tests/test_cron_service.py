...
cron_router_module = importlib.import_module("src.gateway.routers.cron")
...
def _make_store_path() -> Path:
    temp_root = Path.cwd() / ".pytest_runtime_tmp"
    temp_root.mkdir(parents=True, exist_ok=True)
    temp_dir = temp_root / uuid4().hex
    temp_dir.mkdir()
    return temp_dir / "jobs.json"
...
def test_add_job_rejects_invalid_interval_schedule():
    service = CronService(store_path=_make_store_path())
...
def test_add_job_api_returns_422_for_schema_validation_error(monkeypatch):
    service = CronService(store_path=_make_store_path())
...
def test_add_job_api_returns_400_for_no_future_run(monkeypatch):
    service = CronService(store_path=_make_store_path())
    monkeypatch.setattr(cron_router_module, "get_cron_service", lambda: service)
...
def test_enable_job_raises_when_schedule_has_no_future_run():
    service = CronService(store_path=_make_store_path())
    expired_job = service.add_job(
        name="Expired reminder",
    ...
    with pytest.raises(NoFutureRunTimeError, match="no future run time"):
        service.enable_job(expired_job.id, enabled=True)


def test_enable_job_api_returns_409_when_schedule_has_no_future_run(monkeypatch):
    service = CronService(store_path=_make_store_path())
    expired_job = service.add_job(
        name="Expired reminder",
        schedule=CronSchedule(kind="at", at_ms=1),
...
def test_persistence_round_trip_reloads_saved_job(monkeypatch):
    ...
    [saved_job] = reloaded.jobs

    assert saved_job.to_dict() == created.to_dict()


def test_one_time_failed_job_is_disabled_instead_of_deleted():
    async def boom(job):
        del job
