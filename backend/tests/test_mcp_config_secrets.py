...
async def test_update_mcp_configuration_reconciles_tools_cache(monkeypatch, tmp_path):
    reconcile_calls: list[set[str] | None] = []
    ...
    def fake_prepare_mcp_reconciliation(changed):
        reconcile_calls.append(changed)
        return None
    ...
    monkeypatch.setattr(mcp_router, "prepare_mcp_reconciliation", fake_prepare_mcp_reconciliation)
...
def test_delete_mcp_server_route_uses_bodyless_path_parameter(server_name, request_path, monkeypatch):
    ...
    def fake_delete(name: str):
        ...
        changed = {name}
        pending = mcp_router.prepare_mcp_reconciliation(changed)
    ...
    assert reconcile_calls == [{server_name}]
...
async def test_update_mcp_server_state_allows_disabling_but_rejects_enabling_disallowed_command(monkeypatch, tmp_path):
    ...
    assert reconcile_calls == [{"semantic-scholar"}]
