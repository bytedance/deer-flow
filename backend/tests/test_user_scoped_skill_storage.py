...
def user_storage(base_dir: Path, skills_root, config) -> UserScopedSkillStorage:
    ...
    with patch(
        "deerflow.config.paths.get_paths", return_value=Paths(base_dir=base_dir)
    ):
        with patch("deerflow.config.paths._paths", None):
            storage = UserScopedSkillStorage(
                "test-user", host_path=str(skills_root), app_config=config
            )
    return storage
...
class TestPathRedirection:
    ...
    def test_custom_skill_dir_is_user_scoped(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        expected = base_dir / "users" / "test-user" / "skills" / "custom" / "demo-skill"
        assert user_storage.get_custom_skill_dir("demo-skill") == expected

    def test_custom_skill_file_is_user_scoped(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        expected = (
            base_dir
    ...
    def test_history_file_is_user_scoped(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        expected = (
            base_dir
    ...
    def test_public_skill_paths_still_use_global_root(
        self, user_storage: UserScopedSkillStorage, skills_root: Path
    ):
        assert user_storage.get_skills_root_path() == skills_root

    def test_managed_integration_skill_paths_use_global_root(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        assert (
            user_storage.get_integrations_root() == base_dir / "integrations" / "skills"
        )

    def test_user_integrations_root_is_compatibility_alias(
        self, user_storage: UserScopedSkillStorage
    ):
        assert (
            user_storage.get_user_integrations_root()
            == user_storage.get_integrations_root()
        )
...
class TestWriteAndRead:
    ...
    def test_write_creates_file_in_user_dir(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        user_storage.write_custom_skill(
            "demo-skill", "SKILL.md", _skill_content("demo-skill")
        )
        user_file = (
            base_dir
            / "users"
    ...
    def test_write_does_not_create_in_global_custom(
        self, user_storage: UserScopedSkillStorage, skills_root: Path, base_dir: Path
    ):
        user_storage.write_custom_skill(
            "demo-skill", "SKILL.md", _skill_content("demo-skill")
        )
    ...
    def test_write_makes_path_sandbox_readable(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        user_storage.write_custom_skill("demo-skill", "references/ref.md", "# ref")
        skill_dir = (
            base_dir / "users" / "test-user" / "skills" / "custom" / "demo-skill"
        )
...
class TestSkillLoading:
    ...
    def test_public_skills_loaded_from_global(
        self, user_storage: UserScopedSkillStorage, skills_root: Path
    ):
        public_dir = skills_root / "public" / "deep-research"
        public_dir.mkdir(parents=True)
        (public_dir / "SKILL.md").write_text(
            _skill_content("deep-research"), encoding="utf-8"
        )
    ...
    def test_managed_integration_skills_are_global_but_enabled_per_user(
        self, base_dir: Path, skills_root: Path, config
    ):
        integration_dir = base_dir / "integrations" / "skills" / "lark-cli" / "lark-doc"
        integration_dir.mkdir(parents=True)
        (integration_dir / "SKILL.md").write_text(
            _skill_content("lark-doc"), encoding="utf-8"
        )

        with patch(
            "deerflow.config.paths.get_paths", return_value=Paths(base_dir=base_dir)
        ):
            alice = UserScopedSkillStorage(
                "alice", host_path=str(skills_root), app_config=config
            )
            bob = UserScopedSkillStorage(
                "bob", host_path=str(skills_root), app_config=config
            )
        ...
        alice_skill = next(
            skill
            for skill in alice.load_skills(enabled_only=False)
            if skill.name == "lark-doc"
        )
        bob_skill = next(
            skill
            for skill in bob.load_skills(enabled_only=False)
            if skill.name == "lark-doc"
        )
    ...
    def test_public_skill_package_children_are_not_registered(
        self, user_storage: UserScopedSkillStorage, skills_root: Path
    ):
        ...
        (public_dir / "SKILL.md").write_text(
            _skill_content("reviewer"), encoding="utf-8"
        )
        (fixture_dir / "SKILL.md").write_text(
            _skill_content("injection-example"), encoding="utf-8"
        )
    ...
    def test_custom_skills_loaded_from_user_dir(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        user_storage.write_custom_skill(
            "my-skill", "SKILL.md", _skill_content("my-skill")
        )
    ...
    def test_fallback_to_global_custom_when_user_dir_empty(
        self, user_storage: UserScopedSkillStorage, skills_root: Path, base_dir: Path
    ):
        ...
        (global_dir / "SKILL.md").write_text(
            _skill_content("global-skill"), encoding="utf-8"
        )
    ...
    def test_no_fallback_when_user_dir_has_content(
        self, user_storage: UserScopedSkillStorage, skills_root: Path, base_dir: Path
    ):
        ...
        user_storage.write_custom_skill(
            "user-skill", "SKILL.md", _skill_content("user-skill")
        )
    ...
    def test_mixed_public_and_custom(
        self, user_storage: UserScopedSkillStorage, skills_root: Path, base_dir: Path
    ):
        # Create public skill
        public_dir = skills_root / "public" / "deep-research"
...
class TestIsolation:
    ...
    def test_two_users_isolated(self, base_dir: Path, skills_root, config):
        with patch(
            ...
            with patch("deerflow.config.paths._paths", None):
                storage_a = UserScopedSkillStorage(
                    "alice", host_path=str(skills_root), app_config=config
                )
                storage_b = UserScopedSkillStorage(
                    "bob", host_path=str(skills_root), app_config=config
                )

                storage_a.write_custom_skill(
                    "skill-a", "SKILL.md", _skill_content("skill-a")
                )
                storage_b.write_custom_skill(
                    "skill-b", "SKILL.md", _skill_content("skill-b")
                )

                skills_a = [
                    s
                    for s in storage_a.load_skills(enabled_only=False)
                    if s.category == SkillCategory.CUSTOM
                ]
                skills_b = [
                    s
                    for s in storage_b.load_skills(enabled_only=False)
                    if s.category == SkillCategory.CUSTOM
                ]
    ...
    def test_history_per_user(self, base_dir: Path, skills_root, config):
        with patch(
            ...
            with patch("deerflow.config.paths._paths", None):
                ...
                storage_a.write_custom_skill(
                    "shared-name", "SKILL.md", _skill_content("shared-name")
                )

                storage_a.append_history(
                    "shared-name", {"action": "create", "author": "alice"}
                )
...
class TestPathSafety:
    ...
    def test_accepts_skill_files_from_all_allowed_roots(
        self, user_storage: UserScopedSkillStorage, skills_root: Path, base_dir: Path
    ):
        skill_files = [
        ...
        for skill_file in skill_files:
            ...
            skill_file.write_text(
                _skill_content(skill_file.parent.name), encoding="utf-8"
            )

        assert [
            user_storage.validate_skill_file_path(skill_file)
            for skill_file in skill_files
        ] == [skill_file.resolve() for skill_file in skill_files]

    def test_rejects_skill_file_outside_allowed_roots(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
    ...
    def test_accepts_public_skill_file(
        self, user_storage: UserScopedSkillStorage, skills_root: Path
    ):
    ...
    def test_integration_root_accessor_returns_configured_root(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ):
        assert (
            user_storage.get_user_integrations_root()
            == base_dir / "integrations" / "skills"
        )
    ...
    def test_rejects_path_traversal_in_write(
        self, user_storage: UserScopedSkillStorage
    ):
...
class TestFactory:
    ...
    def test_returns_same_instance_for_same_user(
        self, base_dir: Path, skills_root, config
    ):
        with patch(
            "deerflow.config.paths.get_paths", return_value=Paths(base_dir=base_dir)
        ):
    ...
    def test_returns_different_instance_for_different_user(
        self, base_dir: Path, skills_root, config
    ):
        with patch(
            "deerflow.config.paths.get_paths", return_value=Paths(base_dir=base_dir)
...
class TestSkillToggleIsolation:
    ...
    def test_alice_disable_does_not_affect_bob(
        self, base_dir: Path, skills_root, config
    ):
        from types import SimpleNamespace
        ...
        with patch(
            ...
            with patch("deerflow.config.paths._paths", None):
                with patch("deerflow.config.get_app_config", return_value=rich_config):
                    ...
                    storage_alice = get_or_new_user_skill_storage(
                        "alice", app_config=rich_config
                    )
                    storage_bob = get_or_new_user_skill_storage(
                        "bob", app_config=rich_config
                    )

                    # 1. Two users each create a custom skill named "report-gen"
                    storage_alice.write_custom_skill(
                        "report-gen",
                        "SKILL.md",
                        _skill_content("report-gen", "Alice report generator"),
                    )
                    storage_bob.write_custom_skill(
                        "report-gen",
                        "SKILL.md",
                        _skill_content("report-gen", "Bob report generator"),
                    )
                    ...
                    bob_report = [
                        s
                        for s in bob_skills
                        if s.name == "report-gen" and s.category == SkillCategory.CUSTOM
                    ]
                    ...
                    prompt = get_skills_prompt_section(
                        user_id="bob", app_config=rich_config
                    )
                    ...
                    assert (
                        _is_disabled_skill_path(
                            "/mnt/skills/custom/report-gen/SKILL.md", user_id="bob"
                        )
                        is False
                    )
                    ...
                    assert (
                        _is_disabled_skill_path(
                            "/mnt/skills/custom/report-gen/SKILL.md", user_id="alice"
                        )
                        is True
                    )
...
class TestSkillStateAtomicWrite:
    ...
    def test_writes_via_tempfile_then_replace(
        self, user_storage: UserScopedSkillStorage, base_dir: Path
    ) -> None:
        ...
        leftovers = [
            p
            for p in base_dir.glob("users/*/skills/.skill_states_*.json.tmp")
            if p.exists()
        ]
    ...
    def test_failed_write_does_not_truncate_existing_file(
        self, user_storage: UserScopedSkillStorage
    ) -> None:
        import json as _json
        ...
        target.write_text(
            _json.dumps({"old-skill": {"enabled": False}}), encoding="utf-8"
        )
...
class TestSkillStateFailClosed:
    ...
    def test_returns_true_when_state_lookup_raises(self) -> None:
        ...
        with patch(
            "deerflow.skills.storage.user_scoped_skill_storage.UserScopedSkillStorage.get_skill_enabled_state",
            side_effect=_boom,
        ):
            assert (
                _is_disabled_skill_path(
                    "/mnt/skills/custom/report-gen/SKILL.md", user_id="default"
                )
                is True
            )
    ...
    def test_returns_true_when_public_extensions_config_raises(self) -> None:
        ...
        with patch(
            ...
            assert (
                _is_disabled_skill_path(
                    "/mnt/skills/public/bootstrap/SKILL.md", user_id="default"
                )
                is True
            )
...
class TestSkillLoadingRespectsGlobalDisable:
    ...
    def test_global_disable_wins_when_per_user_state_missing(
        self, tmp_path: Path
    ) -> None:
        from types import SimpleNamespace
        ...
        with patch(
            "deerflow.config.paths.get_paths", return_value=Paths(base_dir=base)
        ):
            with patch("deerflow.config.paths._paths", None):
                cfg = SimpleNamespace(
                    skills=SimpleNamespace(
                ...
                with patch("deerflow.config.get_app_config", return_value=cfg):
                    ...
                    ext_cfg = SimpleNamespace(
                        ...
                        is_skill_enabled=lambda name, _cat: (
                            not (name == "shared-skill")
                        ),
                    )
                    ...
                    with patch(
                        ...
                        shared = [
                            ...
                            and s.category == SkillCategory.LEGACY
                        ]
                        assert len(shared) == 1
    ...
    def test_evicts_least_recently_used_above_maxsize(self, monkeypatch) -> None:
        ...
        with (
            patch.object(
                ...
                side_effect=_user_storage,
            ),
            patch.object(
                prompt_module, "get_or_new_skill_storage", side_effect=_global_storage
            ),
        ):
            for i, cfg in enumerate(configs):
                prompt_module.get_enabled_skills_for_config(
                    app_config=cfg, user_id=f"user-{i}"
                )
        ...
        with (
            ...
            prompt_module.get_enabled_skills_for_config(
                app_config=configs[0], user_id="user-0"
            )
        ...
        with (
            ...
            prompt_module.get_enabled_skills_for_config(
                app_config=new_cfg, user_id="user-new"
            )
