"""Migration tests for 0028_conversation_share_quotas (#4548).
...
_REVISION = "0028_conversation_share_quotas"
_PREVIOUS = "0027_conversation_shares"


def test_share_quotas_is_the_single_chain_head() -> None:
...
def test_pins_immediate_parent_revision() -> None:
    """The quota revision follows shares without introducing another branch."""
    import importlib.util
    from pathlib import Path

    module_path = Path(_MIGRATIONS_DIR) / "versions" / "0028_conversation_share_quotas.py"
    spec = importlib.util.spec_from_file_location("migration_0028", module_path)
