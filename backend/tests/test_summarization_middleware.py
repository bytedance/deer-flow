...
def test_skill_rescue_recognizes_skill_load_tool_calls() -> None:
    captured: list[SummarizationEvent] = []
    middleware = _middleware(
        before_summarization=[captured.append],
        trigger=("messages", 4),
...
def test_skill_rescue_normalizes_skill_load_file_paths_for_deduplication() -> None:
    captured: list[SummarizationEvent] = []
    middleware = _middleware(
        before_summarization=[captured.append],
        trigger=("messages", 6),
    ...
    messages = [
        HumanMessage(content="u1"),
        AIMessage(content="", tool_calls=[{"name": "skill_load", "id": "t1", "args": {"skill_name": "demo", "file_path": "references/notes.md"}}]),
        ToolMessage(content="old notes", tool_call_id="t1"),
        HumanMessage(content="u2"),
        AIMessage(content="", tool_calls=[{"name": "skill_load", "id": "t2", "args": {"skill_name": "demo", "file_path": " ./references/notes.md "}}]),
        ToolMessage(content="new notes", tool_call_id="t2"),
        HumanMessage(content="u3"),
        AIMessage(content="final"),
    ]
    ...
    preserved = captured[0].preserved_messages
    assert any(isinstance(m, ToolMessage) and m.content == "new notes" for m in preserved)
    assert not any(isinstance(m, ToolMessage) and m.content == "old notes" for m in preserved)
