...
def test_apply_prompt_template_injects_thread_id(monkeypatch):
    config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=[]),
        skills=SimpleNamespace(container_path="/mnt/skills"),
    )
    ...
    prompt = prompt_module.apply_prompt_template(thread_id="test-thread-001")

    assert "Current thread ID" in prompt
    assert "test-thread-001" in prompt


def test_apply_prompt_template_thread_id_defaults_to_empty(monkeypatch):
    config = SimpleNamespace(
        sandbox=SimpleNamespace(mounts=[]),
        skills=SimpleNamespace(container_path="/mnt/skills"),
    )
    ...
    assert "Current thread ID" in prompt
    assert " — used when generating" in prompt
