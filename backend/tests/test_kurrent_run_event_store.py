...
class FakeRecordedEvent:
    ...
    def __init__(self, *, type: str, data: bytes, metadata: bytes = b"{}", stream_position: int):
        self.type = type
        self.data = data
...
class FakeAsyncKurrentDBClient:
    ...
    async def append_to_stream(self, stream_name, *, events, current_version, timeout=None):
        ...
        for i, ev in enumerate(events):
            existing.append(FakeRecordedEvent(type=ev.type, data=ev.data, metadata=ev.metadata, stream_position=start_position + i))
