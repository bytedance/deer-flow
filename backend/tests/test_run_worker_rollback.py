...
async def test_run_agent_cleans_up_when_mcp_task_projection_is_cancelled():
    class EvictionTrackingRunManager(RunManager):
        def __init__(self) -> None:
            super().__init__()
            self.eviction_calls: list[tuple[str, float]] = []

        def schedule_terminal_eviction(self, run_id: str, *, delay: float = 300, **_kwargs):
            self.eviction_calls.append((run_id, delay))
            return None
    ...
    run_manager = EvictionTrackingRunManager()
    ...
    assert run_manager.eviction_calls == [(record.run_id, 300)]
...
@pytest.mark.anyio
async def test_run_agent_schedules_terminal_run_record_eviction():
    class EvictionTrackingRunManager(RunManager):
        def __init__(self) -> None:
            super().__init__()
            self.eviction_calls: list[tuple[str, float]] = []
...
@pytest.mark.anyio
async def test_run_agent_schedules_terminal_eviction_when_publish_end_fails(monkeypatch):
...
@pytest.mark.anyio
async def test_run_agent_schedules_terminal_eviction_when_completion_hook_is_cancelled(monkeypatch):
    import deerflow.runtime.runs.worker as worker_module
    from deerflow.runtime.journal import RunJournal
...
@pytest.mark.anyio
async def test_terminal_cleanup_and_eviction_tasks_do_not_inherit_run_context():
    marker: ContextVar[str | None] = ContextVar("run_cleanup_marker", default=None)
    seen: dict[str, str | None] = {}
    bridge_cleaned = asyncio.Event()
    manager_eviction_started = asyncio.Event()

    class EvictionTrackingRunManager(RunManager):
        def __init__(self) -> None:
            super().__init__(store=MemoryRunStore())

        async def _evict_terminal_when_safe(self, run_id: str, **_kwargs) -> None:
            del run_id
            seen["manager"] = marker.get()
            manager_eviction_started.set()
    ...
    try:
        ...
        await asyncio.wait_for(
            asyncio.gather(bridge_cleaned.wait(), manager_eviction_started.wait()),
            timeout=1,
        )
    finally:
        marker.reset(token)
