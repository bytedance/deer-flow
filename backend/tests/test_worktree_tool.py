...
@pytest.mark.anyio
async def test_create_coding_worktree_uses_user_selected_target_repository_then_binds(
    ...
    monkeypatch.setattr(
        worktree_module, "ensure_sandbox_initialized_async", fake_ensure_sandbox
    )
    monkeypatch.setattr(worktree_module, "is_local_sandbox", lambda _runtime: True)
    monkeypatch.setattr(
        worktree_module, "resolve_runtime_user_id", lambda _runtime: "alice"
    )
    monkeypatch.setattr(
        worktree_module, "create_task_graph", lambda _thread_id, *, user_id: FakeGraph()
    )
    ...
    assert result == (
        f"Created coding worktree 'coding-run' for {repository} at {worktree} on branch coding/coding-run; bound 3 tasks"
    )


@pytest.mark.anyio
async def test_create_coding_worktree_does_not_bind_when_worktree_verification_fails(
    monkeypatch, tmp_path
):
    repository = tmp_path / "project-a"
    repository.mkdir()
    ...
    assert (
        f"expected path={str(repository.resolve() / '.worktrees' / 'coding-run')!r}"
        in message
    )
...
@pytest.mark.anyio
async def test_create_coding_worktree_reports_actual_repository_check_output(
    monkeypatch, tmp_path
):
    repository = tmp_path / "project-a"
    repository.mkdir()
