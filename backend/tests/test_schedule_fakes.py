...
class TestOptimisticConcurrency:
    """`save` is a compare-and-set on `version`; every other committed write
    bumps it.

    This is what stops a read-modify-write from rolling back the fields the
    dispatch and completion paths own. The API reads a task, the user edits the
    title, and meanwhile the poller launches it -- committing the edited
    snapshot wholesale would restore the pre-launch `next_run_at` and
    `run_count`, re-arming an occurrence that already ran.

    Atomicity is still out of scope here (see the module docstring): these
    cases interleave by hand, single-threaded. What they pin is that both
    implementations agree on *when* a write is refused.
    """

    async def test_a_stored_task_starts_at_version_zero(self, tasks):
        stored = await tasks.add(make_task())
        assert stored.version == 0

    async def test_save_returns_the_incremented_version(self, tasks):
        from dataclasses import replace

        stored = await tasks.add(make_task())
        saved = await tasks.save(replace(stored, title="renamed"))
        assert saved.version == stored.version + 1

    async def test_a_second_save_of_a_stale_snapshot_is_refused(self, tasks):
        from dataclasses import replace

        stored = await tasks.add(make_task())
        await tasks.save(replace(stored, title="first"))

        with pytest.raises(ConcurrentUpdateError):
            await tasks.save(replace(stored, title="second"))

    async def test_the_refused_write_leaves_the_stored_state_alone(self, tasks):
        from dataclasses import replace
        ...
        reloaded = await tasks.get(stored.task_id, user_id="user-1")
        assert reloaded.title == "first"

    async def test_record_launch_invalidates_an_earlier_read(self, tasks):
        """The race the token exists for: a snapshot read before the dispatch
        path committed must not be saveable afterwards."""
        from dataclasses import replace

        stored = await tasks.add(make_task(next_run_at=NOW))
        await tasks.record_launch(
            stored.task_id,
            status=TaskStatus.ENABLED,
        ...
        with pytest.raises(ConcurrentUpdateError):
            await tasks.save(replace(stored, title="renamed"))

        reloaded = await tasks.get(stored.task_id, user_id="user-1")
        assert reloaded.run_count == 1, "the launch write survived"
        assert reloaded.last_run_id == "run-1"

    async def test_record_completion_invalidates_an_earlier_read(self, tasks):
        from dataclasses import replace

        stored = await tasks.add(make_task())
        await tasks.record_completion(stored.task_id, user_id="user-1", status=None, error="boom")

        with pytest.raises(ConcurrentUpdateError):
            await tasks.save(replace(stored, title="renamed"))

    async def test_claim_due_invalidates_an_earlier_read(self, tasks):
        from dataclasses import replace

        stored = await tasks.add(make_task(next_run_at=NOW - timedelta(minutes=1)))
        claimed = await tasks.claim_due(now=NOW, lease_seconds=60, limit=10)
        assert [task.task_id for task in claimed] == [stored.task_id]

        with pytest.raises(ConcurrentUpdateError):
            await tasks.save(replace(stored, title="renamed"))

    async def test_a_freshly_read_snapshot_saves_again(self, tasks):
        """The token refuses stale writes, not repeated ones -- re-reading is
        the whole recovery path, so it has to work."""
        from dataclasses import replace

        await tasks.add(make_task())
        for title in ("first", "second", "third"):
            current = await tasks.get("task-1", user_id="user-1")
            await tasks.save(replace(current, title=title))

        assert (await tasks.get("task-1", user_id="user-1")).title == "third"

    async def test_an_absent_task_is_none_not_a_conflict(self, tasks):
        """ "No such task" and "someone else wrote first" are different answers;
        the caller maps them to 404 and 409 respectively."""
        assert await tasks.save(make_task(task_id="task-absent")) is None

    async def test_another_users_task_is_none_not_a_conflict(self, tasks):
        await tasks.add(make_task())
        stored = await tasks.get("task-1", user_id="user-1")

        from dataclasses import replace

        assert await tasks.save(replace(stored, user_id="user-2")) is None
