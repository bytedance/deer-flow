...
class TestMarkdownMemoryStorage:
    ...
    def _markdown_storage_at(self, memory_file) -> MarkdownMemoryStorage:
        return MarkdownMemoryStorage(DeerMemConfig(storage_path=str(memory_file.resolve())))
    ...
    def test_corrupt_json_with_fenced_block_recovers(self, tmp_path):
    ...
    def test_fenced_block_containing_backticks_parses_losslessly(self):
        """Remembered strings may contain ``` (code snippets): the greedy
        fence match must keep the JSON round-trip lossless."""
        manifest = create_empty_memory()
