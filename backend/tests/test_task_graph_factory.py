...
def test_create_task_graph_uses_thread_scoped_storage_and_recovers_tasks(tmp_path):
    ...
    assert (
        ...
        == paths.thread_dir("thread-1", user_id="alice") / "coding-tasks"
    )
...
def test_create_task_graph_isolates_users_and_threads(tmp_path):
    ...
    alice_first.add_tasks(
        [CodingTask(id="task-1", subject="Alice first", description="First thread")]
    )
    alice_second.add_tasks(
        [CodingTask(id="task-1", subject="Alice second", description="Second thread")]
    )
    bob_first.add_tasks(
        [CodingTask(id="task-1", subject="Bob first", description="Different user")]
    )
...
def test_task_graph_recovers_failed_dependent_task_across_instances(tmp_path):
    ...
    graph.add_tasks(
        [
            CodingTask(
                id="coding-analysis", subject="Analyze", description="Analyze the code"
            ),
            CodingTask(
