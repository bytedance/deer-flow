        ...
        HumanMessage(
            ...
            additional_kwargs={
                "human_input_response": {
                    ...
                    "option_id": "option-1"
                    if response_value == RETRY_OPTION
                    else "option-2",
...
def _runtime(messages: list, *, thread_id: str | None = "thread-1") -> SimpleNamespace:
    ...
    return SimpleNamespace(
        context=context, config={"configurable": {}}, state={"messages": messages}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        _approval_messages(
            "coding-implementation", response_request_id="another-request"
        ),
    ],
    ids=["missing", "wrong-task", "rejected", "wrong-request"],
)
def test_recover_coding_task_rejects_missing_or_mismatched_approval(
...
def test_recover_coding_task_requires_thread_id():
    with pytest.raises(ValueError, match="thread_id is required"):
        recover_coding_task.func(
            coding_task_id="coding-implementation",
            runtime=_runtime(
                _approval_messages("coding-implementation"), thread_id=None
            ),
        )
