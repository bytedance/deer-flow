...
class TestTaskMode:
    ...
    def test_sync_outputs_to_storage(self, mock_config):
        ...
        outputs_backend = MagicMock()
        workspace_backend = MagicMock()
        with patch(
            "src.community.e2b_sandbox.e2b_sandbox_provider.get_thread_file_backend",
            side_effect=lambda feature: outputs_backend if feature == "outputs" else workspace_backend,
        ):
            provider.sync_outputs_to_storage("sbx-123", "thread-abc")

        # Should write output files through thread-file backend
        outputs_backend.put_virtual_file.assert_called_once_with("thread-abc", "/mnt/user-data/outputs/chart.png", b"\x89PNG")
