...
def _make_state_with_usage(total: int, input_tk: int = 0, output_tk: int = 0, tool_calls=None, content=""):
    ...
    msg = AIMessage(content=content, tool_calls=tool_calls or [], usage_metadata={"input_tokens": input_tk, "output_tokens": output_tk, "total_tokens": total})
    return {"messages": [msg]}
...
class TestTokenBudgetTracking:
    ...
    def test_below_threshold_returns_none(self):
        config = TokenBudgetConfig(max_tokens=100000, warn_threshold=0.8)
        mw = TokenBudgetMiddleware.from_config(config)
    ...
    def test_warning_threshold_injects_warning_and_returns_none(self):
        ...
        msg1 = AIMessage(content="1", usage_metadata={"total_tokens": 45000, "input_tokens": 0, "output_tokens": 0})
        msg2 = ToolMessage(content="ok", tool_call_id="call1")
        msg3 = AIMessage(content="3", usage_metadata={"total_tokens": 45000, "input_tokens": 0, "output_tokens": 0})
        ...
        assert result is None
        assert len(mw._pending_warnings[("test-thread", "test-run")]) == 1
        assert "TOKEN BUDGET WARNING" in mw._pending_warnings[("test-thread", "test-run")][0]
...
class TestTokenBudgetHardStop:
    def test_hard_stop_strip_tool_calls(self):
        config = TokenBudgetConfig(max_tokens=100000, hard_stop_threshold=1.0)
        mw = TokenBudgetMiddleware.from_config(config)
...
class TestIndependentDimensions:
    def test_input_tokens_trigger_limit(self):
        config = TokenBudgetConfig(max_tokens=100000, max_input_tokens=10000, warn_threshold=0.8)
        mw = TokenBudgetMiddleware.from_config(config)
        ...
        warnings = mw._pending_warnings[("test-thread", "test-run")]
        assert len(warnings) == 1
        assert "input token" in warnings[0]

    def test_output_tokens_trigger_limit(self):
        config = TokenBudgetConfig(max_tokens=100_000, max_output_tokens=5_000, hard_stop_threshold=1.0)
        mw = TokenBudgetMiddleware.from_config(config)
