...
def test_coding_task_is_claimed_before_delegation_and_completed_after_success(monkeypatch):
    calls: list[tuple] = []
...
def test_coding_task_worktree_is_passed_as_subagent_workspace_without_mutating_parent(monkeypatch):
    calls: list[tuple] = []
...
def test_coding_task_worktree_preparation_failure_marks_claimed_task_failed(monkeypatch):
    calls: list[tuple] = []
    ...
    assert calls == [
        ("claim", "task-1", "code-analyzer"),
        ("fail", "task-1", "Failed to start subagent: thread_data is required when using a coding worktree"),
    ]
