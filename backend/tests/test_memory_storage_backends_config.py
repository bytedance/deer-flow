"""Bridge-level config tests for PG/Mongo memory storage backends."""
...
def test_get_memory_storage_postgres_backend_requires_psycopg_and_falls_back_when_missing():
    cfg = MemoryConfig(
    ...
    with patch("deerflow.agents.memory.storage.get_memory_config", return_value=cfg):
        ...
        # In this environment psycopg may be absent, so the loader must fail safe.
        assert storage is not None


def test_get_memory_storage_mongo_backend_requires_pymongo_and_falls_back_when_missing():
    cfg = MemoryConfig(
...
def test_memory_config_exposes_bridge_fields(field: str, value: str):
