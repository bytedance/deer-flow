...
def test_can_start_requires_every_dependency_to_exist_and_be_completed(task_graph):
    ...
    store.save(CodingTask(id="task-2", subject="Implement", description="Implement change", blocked_by=["task-1"]))
...
def test_claim_rejects_non_pending_or_blocked_task(task_graph):
    ...
    store.save(CodingTask(id="task-2", subject="Blocked", description="Wait for missing dependency", blocked_by=["missing-task"]))
...
def test_complete_persists_status_and_returns_newly_ready_task_ids(task_graph):
    ...
    store.save(CodingTask(id="task-4", subject="Still blocked", description="Missing dependency", blocked_by=["missing-task"]))
...
def test_add_tasks_validates_the_whole_batch_before_persisting(task_graph):
    graph, store = task_graph
    tasks = [
        CodingTask(id="task-2", subject="Implement", description="Implement change", blocked_by=["task-1"]),
...
def test_add_tasks_rejects_missing_dependencies_without_persisting(task_graph):
    graph, store = task_graph
    tasks = [CodingTask(id="task-1", subject="Implement", description="Implement change", blocked_by=["missing-task"])]
...
@pytest.mark.parametrize(
    "tasks",
    [
        [CodingTask(id="task-1", subject="Analyze", description="Analyze change", blocked_by=["task-1"])],
        [
            CodingTask(id="task-1", subject="Analyze", description="Analyze change", blocked_by=["task-2"]),
            CodingTask(id="task-2", subject="Implement", description="Implement change", blocked_by=["task-1"]),
        ],
    ],
