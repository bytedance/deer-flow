...
def test_open_session_enables_subagent_delegation():
    with (
        ...
        patch("deerflow.runtime.checkpointer.provider.get_checkpointer", return_value=sentinel.checkpointer),
    ):
        session = open_session(persistence=False)

    client_cls.assert_called_once_with(checkpointer=sentinel.checkpointer, subagent_enabled=True)
