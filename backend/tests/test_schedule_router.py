...
class TestToCommand:
    ...
    wire; the wire's `None` = "not supplied" convention carries straight into
    the command, whose fields read `None` the same way."""
    ...
    def test_update_carries_omitted_fields_as_none(self):
...
class TestErrorMapping:
    ...
    @pytest.mark.asyncio
    async def test_a_lost_optimistic_write_is_a_retryable_409(self):
        """The service already retried a bounded number of rounds, so reaching
        the router means the row kept moving. 409 tells the client the one
        thing it can act on: read again and resubmit."""
        from deerflow.domain.schedule.exceptions import ConcurrentUpdateError

        @router_module._map_domain_errors
        async def handler():
            raise ConcurrentUpdateError("task-1 was modified concurrently")
