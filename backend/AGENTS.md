...
- Once `_launch_run` returns, later bookkeeping failures remain launch outcomes: `dispatch_task` retries the run-row and task-row writes without marking the active row terminal, returns the real `run_id`, and leaves a queued row active if recovery is still unavailable. `ScheduledTaskRepository.update_after_launch` is idempotent for the same `last_run_id`, so retrying an uncertain commit cannot increment `run_count` twice. The completion hook can still backfill the task-run row from run metadata.
