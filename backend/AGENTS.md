...
**Persistent CodingTask binding**: `task()` accepts an optional `coding_task_id`. When present, it resolves the current user/thread TaskGraph, claims that task with the selected `subagent_type` before dispatch, and completes it only after a successful terminal subagent result. A task with `agent_type` rejects claims from a different profile. Completed coding profiles must return the Pydantic-validated JSON type declared by `SubagentConfig.artifact_type`; the normalized artifact is persisted on `CodingTask.artifact`, and `TaskGraph.get_upstream_artifacts()` supplies completed transitive dependency artifacts to the next profile as explicitly delimited untrusted data. `create_coding_worktree(repository_path=...)` is a trusted-local feature: each run supplies the absolute path of the user-selected repository, which may live on any local drive; the tool creates `.worktrees/{name}` there, verifies the resulting Git worktree and branch, and persists the full host Worktree path in `CodingTask.worktree`. A bound path is projected directly into a copied `thread_data.workspace_path` before `SubagentExecutor` construction, so only the delegated agent enters the target project's Worktree and the parent's runtime state is not mutated. Profiles with `workspace_access="read_only"` receive no direct file-write tools and must leave the Git-visible Worktree fingerprint unchanged; a persistent tracked or non-ignored untracked change fails the CodingTask. This is an outcome-integrity check, not an OS-level immutable mount. Non-local providers fail explicitly instead of silently operating on the per-thread DeerFlow workspace. Failed, cancelled, timed-out, disappeared, Worktree-preparation, executor-construction, launch-error, output-contract, read-only-integrity, parent-cancelled, and unexpected-exception paths call the same CodingTask failure transition and persist `last_failure_reason`. Recovery resets only status and owner; the next delegated agent receives that reason as diagnostic context and must verify the current Worktree and diff. `recover_coding_task` calls `TaskGraph.recover()` only after finding a structured `ask_clarification` option response whose request ID, risk-confirmation context (`coding_task_recovery:{task_id}`), offered option, and task ID all match; this is a human-in-the-loop interaction gate, not a production authorization boundary. Calls without `coding_task_id` preserve the original delegation behavior.
**Coding profiles and skill wiring**: `subagents/builtins/coding_agents.py` registers `code-analyzer`, `code-implementer`, and `code-reviewer` as first-class built-ins with separate prompts, tool allowlists, workspace access, and report contracts. `skills/public/multi-agent-coding/SKILL.md` allows `ask_clarification`, `submit_task_plan`, `create_coding_worktree`, `recover_coding_task`, `continue_after_review`, and `task`. After its input gate, it requests explicit plan approval before persisting the selected role-bound DAG: `analyze_only`, `review_only`, or `coding-analysis -> coding-implementation -> coding-review`, then creating the selected tasks' shared Worktree. `submit_task_plan` also persists the approved `coding_brief` as a thread-level `CodingRunPlan`; each delegated task reloads and injects it before its own task prompt, so context compaction cannot replace the accepted goal with a summary. The task layer injects validated upstream reports automatically. Every technical delegation failure asks for a fresh retry decision before recovery and resubmission; no autonomous retry loop is allowed.
**Workflow selection and review follow-up**: `multi-agent-coding` selects one of `analyze_only`, `review_only`, or `implement_and_review`; each selected workflow receives a shared isolated Worktree. The first two are single-role, read-only runs. A `review_report` with verdict `FAIL` is a completed review result rather than a failed `CodingTask`, so it cannot use `recover_coding_task`. After a matching `ask_clarification` approval with context `coding_review_followup:{review_task_id}`, `continue_after_review` validates that completed FAIL artifact, appends analyzer → implementer → reviewer follow-up nodes to the existing TaskGraph, binds them to the existing Worktree, and updates `CodingRunPlan.task_ids`. `TaskGraph.add_tasks()` validates the combined persisted and new graph so the follow-up may depend on a historical review task. The Skill must not automatically create this branch or loop; every review failure needs fresh human approval.
...
- Terminal sessions created by `tui/session.py::open_session()` enable subagent delegation, so the embedded TUI and headless CLI expose the `task` tool and configured subagents.
