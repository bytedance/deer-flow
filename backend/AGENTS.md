...
**Benefit-based routing policy**: Enabling subagents exposes delegation as an optimization, not a default response to complexity. The lead prompt defaults to direct execution and permits `task` only when parallel latency, specialist capability, or context-isolation benefit clearly exceeds startup, duplicate-discovery, synthesis, state-conflict, and side-effect costs. Sequential dependencies and overlapping mutable state are hard vetoes; parallel scopes must be independent and non-overlapping, the lead uses the fewest useful subagents, and every later batch is re-evaluated rather than pre-planned automatically. Keep this policy aligned across `lead_agent/prompt.py`, the `task` tool description, and built-in role descriptions; routing regressions are pinned in `tests/test_subagent_prompt_security.py` and `tests/test_lead_agent_prompt.py`.
