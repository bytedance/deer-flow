...
| **Uploads** (`/api/threads/{id}/uploads`) | `POST /` - atomically publish files without replacing collisions and optionally convert PDF/PPT/Excel/Word; `GET /list` - list primary uploads only; `DELETE /{filename}` - wait for that filename's active publication lifecycle, then delete the primary and its exact generated conversion |
...
**Virtual Path System**:
- Agent sees: `/mnt/user-data/{workspace,uploads,outputs}`, system-owned `/mnt/user-data/.upload-conversions`, and `/mnt/skills`
