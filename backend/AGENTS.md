...
- **Gateway custom-skill IO**: custom skill content and rollback history live on the host filesystem. Async get/edit/rollback routes must keep existence probes, history reads, `SKILL.md` reads, and content validation inside `asyncio.to_thread`; security scans remain awaited in their original order between the rollback target snapshot and the current-content snapshot.
