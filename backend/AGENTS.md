...
- `run_agent()` and checkpoint-state accessor construction run the synchronous agent factory through `asyncio.to_thread`, keeping lazy MCP/tool assembly waits off the Gateway event loop; `tests/blocking_io/test_agent_factory_construction.py` pins the heartbeat guarantee for both paths.
