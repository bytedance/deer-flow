...
**Runtime**:
- `make dev`, Docker dev, and production all run the agent runtime in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/deerflow/runtime/`). Nginx exposes that runtime at `/api/langgraph/*` and rewrites it to Gateway's native `/api/*` routers. Terminal runs evict their in-memory registry record event-driven via `RunManager.evict_finished_run()`: awaited at the tail of `run_agent`'s finally block (after every finalization step — durable terminal status, receipts, duration, observers, finalizing-barrier release, end frame), and from `fail_start_if_pending` and `_close_cancelled_admission`. Eviction first confirms the terminal state in the RunStore (repairing the row from the in-memory snapshot when the best-effort terminal writes failed) and retains the record for bounded retries while the store cannot confirm; there is no eviction without a RunStore backing (no fallback for `get`/`list_by_thread`, and the registry is the only idempotency dedup). Idempotent-reuse hydrations of store rows are returned uncached — a cached active row owned by another worker would outlive the owner's completion and fence the thread via `local_inflight` forever. The cancel path keeps interrupted runs idempotently cancellable (202) after eviction by recognizing the durable store row, and the RunStore stays the source of truth for run history, so the in-process registry (which retains each run's full `kwargs["input"]`) cannot grow without bound.
