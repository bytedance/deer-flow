...
**Persistent CodingTask binding**: `task()` accepts an optional `coding_task_id`. When present, it resolves the current user/thread TaskGraph, claims that task with the selected `subagent_type` before dispatch, and completes it only after a successful terminal subagent result. `create_coding_worktree(repository_path=...)` is a trusted-local feature: each run supplies the absolute path of the user-selected repository, which may live on any local drive; the tool creates `.worktrees/{name}` there, verifies the resulting Git worktree and branch, and persists the full host Worktree path in `CodingTask.worktree`. A bound path is projected directly into a copied `thread_data.workspace_path` before `SubagentExecutor` construction, so only the delegated agent enters the target project's Worktree and the parent's runtime state is not mutated. Non-local providers fail explicitly instead of silently operating on the per-thread DeerFlow workspace. Failed, cancelled, timed-out, disappeared, Worktree-preparation, executor-construction, launch-error, parent-cancelled, and unexpected-exception paths call the same CodingTask failure transition and persist a reason. `recover_coding_task` calls `TaskGraph.recover()` only after finding a structured `ask_clarification` option response whose request ID, risk-confirmation context (`coding_task_recovery:{task_id}`), offered option, and task ID all match; this is a human-in-the-loop interaction gate, not a production authorization boundary. Calls without `coding_task_id` preserve the original delegation behavior.
**Coding skill wiring**: `skills/public/multi-agent-coding/SKILL.md` allows `ask_clarification`, `submit_task_plan`, `create_coding_worktree`, `recover_coding_task`, and `task`. After its input gate, it requests explicit plan approval before persisting the stable `coding-analysis -> coding-implementation -> coding-review` stage DAG or creating their shared Worktree. Every failed delegation asks for a fresh retry decision before recovery and resubmission; no autonomous retry loop is allowed.
