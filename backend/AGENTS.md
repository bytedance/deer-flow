...
**Step capture & persistence (#3779)**: `executor.py` captures both assistant turns (`AIMessage`) **and** tool outputs (`ToolMessage`) via `subagents/step_events.py::capture_step_message`, so `task_running` streams "what each step produced", not just tool-call requests. `runtime/runs/worker.py::_persist_subagent_event` additionally persists these `task_*` custom events to the `RunEventStore` as `subagent.start`/`subagent.step`/`subagent.end` (`category="subagent"`, `task_id` in `metadata`, per-step text capped at `SUBAGENT_STEP_MAX_CHARS`). The dedicated category keeps them out of `list_messages` (the thread feed) while `list_events` returns them for the frontend's fetch-on-expand backfill. `step_events.py` is a pure, unit-tested layer (`build_subagent_step` / `subagent_run_event`).
