...
- Scheduled-task IM result push (issue #4254): when `channel_connections.enabled` is true **and** a channel service is running, terminal scheduled runs enqueue durable rows into `notification_deliveries` (migrations `0014`/`0015`) for each of the owner's connected IM identities. `NotificationDeliveryWorker` (`app/scheduler/notification_delivery.py`) polls on the scheduler's `poll_interval_seconds`, claims due rows atomically, and pushes via `Channel.send_notification`. Execution status is written first and never shadowed by delivery failures. Manual triggers and interrupts stay silent. Channel/transport outages park with `count_attempt=False` (`ChannelUnavailable`); platform rejections count against the retry budget. Enqueue and the worker activate together — no write-only outbox when channels are down. Proactive push is implemented for WeCom today; other providers keep the base default (raises) so the outbox records a failure instead of silently dropping. See `app/channels/AGENTS.md` for the `send_notification` contract and `persistence/migrations/AGENTS.md` for migrations `0014`/`0015`.
