...
`GET/PATCH /api/v1/auth/preferences` persists only notification enablement, the
default model, conversation mode, and reasoning effort. It requires a browser
session plus `X-Expected-User-Id` matching that session (a stale-tab guard, never
an authorization source); PAT, internal, and auth-disabled callers are rejected.
...
Never persist the arbitrary agent context or credentials through this API.
