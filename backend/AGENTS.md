...
- **Installation**: `POST /api/skills/install` extracts a `.skill` ZIP archive to the custom directory; `POST /api/skills/install/upload` accepts a direct multipart offline package upload. Both routes are admin-only, run the same archive validation and security scans, install into the user-scoped custom directory, and refresh that user's skill prompt cache. Direct uploads authorize before multipart parsing, enforce a streaming 513 MiB request ceiling (512 MiB archive plus framing), and use matching exact-route limits in the Docker, local, and Helm nginx configs.
