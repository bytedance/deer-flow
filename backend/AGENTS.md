...
- Once-task recovery reads the deterministic latest run row (`created_at`, then `scheduled_for`, then `id`) and finalises the parent from terminal outcomes: success→completed, failed→failed, interrupted/skipped→cancelled; active rows stay running for a later pass.
