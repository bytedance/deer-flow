...
Admin user management is an authenticated browser-admin boundary and is unavailable whenever `DEER_FLOW_AUTH_DISABLED=1`; this explicit route gate also prevents a retained real session cookie from reaching a CSRF-exempt mutation in auth-disabled mode. `GET /api/v1/admin/users?offset&limit` returns `{users, total}` for existing local and OIDC accounts, and `PATCH /api/v1/admin/users/{id}/role` promotes or demotes a user. Regular users cannot use the route to promote themselves. The repository transaction serializes role changes across workers and rejects a change that would leave no administrator. Only a real role transition increments the target's existing `token_version`; the old cookie then fails at the next HTTP, LangGraph, or new connection authentication boundary and the affected user must sign in again. Requests and already-established long-lived connections are not actively interrupted. Role changes produce a keyed security-event log message rather than persistent audit rows, so this feature reuses the existing `users.system_role` / `users.token_version` columns and adds no schema migration.
