...
- **SkillScan**: `packages/harness/deerflow/skills/skillscan/` is the native deterministic scanner for `.skill` archives and agent-managed skill writes. It runs offline before the LLM scanner, emits structured findings (`rule_id`, `severity`, `file`, `line`, `message`, `remediation`, redacted `evidence` — category/analyzer are encoded in the `rule_id` prefix), blocks `CRITICAL`, and passes warning findings into `scan_skill_content()`. `scan_archive_preflight()` / `scan_skill_dir()` are pure sync functions (dispatch off the event loop); `enforce_static_scan()` applies the blocking policy and the `skill_scan.enabled` kill switch. The Python instance-client signal is a one-level lexical-scope analysis over ordinary statement order, deliberately **not** a control-flow interpreter (PR #4265 review): a known client constructor bound to a simple name, name-to-name alias propagation, rebinding invalidation, and a sink recorded at the call so a later rebind cannot retract it. A compound statement (`if`, `try`, `except*`, loop, `match`) has every name it may bind dropped *before* its bodies are walked, and each body is walked from an isolated copy — bodies are analyzed so wrapping code in `if True:` is not a bypass, but no branch is joined, ordered, or tested for feasibility. Nested scopes inherit the outer handle state at their definition position; resolving closures at call time is outside this lexical model. Because the signal feeds a `CRITICAL` rule that hard-blocks installation, every ambiguous case resolves toward not reporting; the main blind spots are documented in #4296 and accepted false negatives, including define-then-bind closure capture, are pinned by `test_python_declared_false_negatives_stay_unreported`. Deep untrusted ASTs may skip only this client-handle signal; findings already collected by deterministic analyzers are retained. Do not add Semgrep/OpenGrep or YAML rule-engine dependencies to the core path; Phase 1 rule specs live in Python constants next to their analyzers in `skillscan/orchestrator.py`.
