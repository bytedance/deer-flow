...
**Runtime**:
- `make dev`, Docker dev, and production all run the agent runtime in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/deerflow/runtime/`). Nginx exposes that runtime at `/api/langgraph/*` and rewrites it to Gateway's native `/api/*` routers. Terminal runs schedule a delayed eviction (`RunManager.schedule_cleanup`, 300s) of their in-memory registry record from `run_agent`'s finally block, `fail_start_if_pending`, and terminal idempotent-reuse hydrations — the RunStore stays the source of truth for run history, so the in-process registry (which retains each run's full `kwargs["input"]`) cannot grow without bound.
