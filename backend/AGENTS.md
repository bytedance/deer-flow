...
- **Per-tool enablement**: `mcpServers.<server>.tools.<original_tool_name>.enabled` defaults to `true`. `get_mcp_tools()` resolves the original MCP name while the producing server is still known and drops a disabled tool before routing metadata, stdio wrapping, cache exposure, deferred-tool assembly, or agent binding. Configure this with the server's original name, not the default `<server_name>_` bound name. After non-empty discovery, configured names that do not match an offered original name warn without failing server initialization; empty/failed discovery does not emit one warning per override. Unknown fields inside an override remain accepted for compatibility but warn with the server and tool context, so typos such as `enable` do not look like a successful disable.
