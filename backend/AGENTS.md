...
- Scheduled occurrences use a parent-locked `occurrence_seq` for recovery and parent-write eligibility; older launches still reconcile their own history but cannot replace newer parent results. `launch_accounted` and `run_count` commit together; completion writes occurrence, accounting, and eligible parent outcome atomically. These fields stay internal. Unsequenced rows (legacy history or a pre-upgrade writer) keep best-effort timestamp/last-run inference and win the latest-run lookup only when newer than the sequence winner by timestamp, so a rolling upgrade degrades to the previous ordering instead of misordering; no historical backfill.
