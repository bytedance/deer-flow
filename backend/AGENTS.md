...
| **Memory** (`/api/memory`) | `GET /` - memory data; `POST /reload` - force reload; `GET /config` - config; `GET /status` - config + data. The data endpoints (get/reload/clear/export/import and fact CRUD) accept an optional `agent_name` query parameter selecting the per-agent fact bucket (validated against the public custom-agent name grammar, 422 on malformed names; omitted selects the default bucket). Summaries are user-global and shared across agents; a scoped clear removes only that agent's facts and preserves the shared summaries |
...
- The `/api/memory*` data endpoints accept an optional `agent_name` query parameter that selects the per-agent fact bucket for reads and fact CRUD (the router validates it against the public custom-agent name grammar and threads it into the backend-neutral `MemoryManager` contract; omitted selects the reserved `__default__` bucket, which never travels the wire). User/history summaries are user-global: they are returned unchanged for every bucket, a scoped clear removes only that bucket's facts, and an import's facts land in the selected bucket while its summaries overwrite the shared ones
