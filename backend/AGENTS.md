...
10. **ToolErrorHandlingMiddleware** - Receives `AppConfig`, converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting, stamps structured metadata for task exception wrappers, and stamps skill-read metadata for downstream durable-context capture. Task tool result text is generated from the same status/result/error inputs as the structured metadata so callers do not hand-write a second protocol string.
...
13. **DurableContextMiddleware** - Captures durable facts from structured tool metadata: `task` tool results carry bounded task status/result/error metadata from the task runtime boundary, and successful `SKILL.md` reads carry a compact `skill_context_entry` from the configured read-tool boundary. If a skill-read candidate lacks matching structured metadata, capture stays empty and logs a warning instead of reparsing the `SKILL.md` body. Transcript text remains model-visible display content, not the durable-context protocol.
