...
- Extends `AgentState` with: `sandbox`, `thread_data`, `title`, `artifacts`, `todos`, `uploaded_files`, `viewed_images`, `promoted`, `delegation_ledger`, `skill_context`, `summary_text`
- Uses custom reducers: `merge_artifacts` (deduplicate), `merge_viewed_images` (merge/clear), `merge_promoted` (catalog-hash-scoped deferred tool promotions), `merge_delegation_ledger` (append task delegation entries, same id latest wins), and `merge_skill_context` (dedupe active-skill references by path, keep the most recently read entries; entries store a name/path/description reference, not the SKILL.md body). `summary_text` is a LastValue channel updated by summarization and injected into model requests as durable context instead of being stored as a `messages` item.
...
13. **DurableContextMiddleware** - Captures completed `task` delegations into `ThreadState.delegation_ledger` and loaded skill-file references (name/path/description, parsed in-memory - not the body) into `ThreadState.skill_context` before summarization can compact the paired tool-call/result messages, then injects durable context (`summary_text` + rendered delegation ledger + an active-skills reminder pointing at each SKILL.md for on-demand re-read) into each model request so compressed history, completed work, and which skills are active stay visible without being stored as `messages`
