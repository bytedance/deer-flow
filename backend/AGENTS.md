...
- The public `DeerFlowClient` assembly path must preserve the same subagent option precedence as Gateway: an explicit call override wins, otherwise a custom agent's `subagent_enabled` and `max_concurrent_subagents` defaults apply before runtime defaults. Keep omitted values absent until assembly so explicit `false` remains distinguishable from unset.
