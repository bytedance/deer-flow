...
10. **ToolErrorHandlingMiddleware** - Receives `AppConfig`, converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting, stamps structured metadata for task exception wrappers, and stamps skill-read metadata for downstream durable-context capture
...
13. **DurableContextMiddleware** - Captures durable facts from structured tool metadata: `task` tool results carry bounded task status/result/error metadata from the task runtime boundary, and successful `SKILL.md` reads carry a compact `skill_context_entry` from the configured read-tool boundary. Transcript text remains model-visible display content, not the durable-context protocol.
