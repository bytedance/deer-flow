...
**Coding profiles and skill wiring**: `subagents/builtins/coding_agents.py` registers `code-analyzer`, `code-implementer`, and `code-reviewer` as first-class built-ins with separate prompts, tool allowlists, workspace access, and report contracts. `skills/public/multi-agent-coding/SKILL.md` allows `ask_clarification`, `submit_task_plan`, `create_coding_worktree`, `recover_coding_task`, and `task`. After its input gate, it requests explicit plan approval before persisting the stable, role-bound `coding-analysis -> coding-implementation -> coding-review` stage DAG or creating their shared Worktree. `submit_task_plan` also persists the approved `coding_brief` as a thread-level `CodingRunPlan`; each delegated task reloads and injects it before its own task prompt, so context compaction cannot replace the accepted goal with a summary. The task layer injects validated upstream reports automatically. Every failed delegation asks for a fresh retry decision before recovery and resubmission; no autonomous retry loop is allowed.
