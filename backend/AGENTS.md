...
- `run_agent()` constructs the synchronous agent factory through `asyncio.to_thread`, keeping lazy MCP/tool assembly waits off the Gateway event loop; `tests/blocking_io/test_agent_factory_construction.py` pins the heartbeat guarantee.
