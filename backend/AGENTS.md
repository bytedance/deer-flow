...
`scripts/benchmark/deermem_scope_isolation/` evaluates DeerMem's automatic
long-term-memory admission and `(user_id, agent_name)` fact isolation. Keep its
semantic path on the public DeerMem queue, updater, deterministic write gate,
and file storage; do not replace that path with a benchmark-local runtime. Its
identity probes must read the same durable write through distinct scopes. The
committed protocol contains synthetic canaries only, pins the production
extraction-prompt hash, and never stores raw prompts or conversation text in
public result rows. Prompt or protocol changes must fail closed until their
expected classifications are reviewed.

Run its offline checks from `backend/`:
...
uv run pytest tests/test_bench_deermem_scope_isolation_*.py -q
```
