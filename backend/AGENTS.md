...
29. **Custom middlewares** - *(optional)* Any `custom_middlewares` passed to `build_middlewares` are injected here, before the deliverable/terminal-response/safety/clarification tail
30. **SlashSkillDeliverableMiddleware** - When a slash-activated skill declares `required-outputs` in `SKILL.md` frontmatter, checks that each basename exists under `thread_data.outputs_path` before the turn can END (for `{skill}.json` requires packager shape: `source` + `schema_version` + `generated_at`, plus step-specific fields such as `title`/`content_html` for article pipeline steps). Missing/invalid files get a hidden packaging reminder and `jump_to=model` (default 2 recoveries); exhaustion stamps `deerflow_error_fallback`. `after_agent` re-checks as a last-chance denial so a recovery loop cannot end as success with a forged shell. Lead-run completion contract, same pattern as #4027 / related to #4176 false success without skill package output. Activation carries `__slash_skill_name` + `__slash_skill_required_outputs` on run context; skills without `required-outputs` are unaffected
...
Additional providers also live here (`bocha`, `boxlite`, `brave`, `browserless`, `crawl4ai`, `ddg_search`, `e2b_sandbox`, `exa`, `fastcrw`, `groundroute`, `infoquest`, `searxng`, `serper`); see each subpackage for specifics.
...
- **Format**: Directory with `SKILL.md` (YAML frontmatter: name, description, license, allowed-tools, required-secrets, required-outputs, secrets-autonomous)
...
Optional slash-activation completion contract: a skill lists output **basenames** that must exist under the thread `outputs/` directory before the lead run may END successfully.

- **Declare**: `required-outputs:` YAML list of basenames only (no path separators), e.g. `content-research.json`. Parsed by `skills/parser.py::parse_required_outputs` into `Skill.required_outputs`.
- **Enforce**: `SkillActivationMiddleware` copies the list onto run context (`__slash_skill_required_outputs` with `__slash_skill_name`). `SlashSkillDeliverableMiddleware` checks the files on terminal model turns — retry with a packaging reminder, then `deerflow_error_fallback` (same lead-run failure channel as #4027; addresses the #4176 class of false success without a package). Skills that omit the field are unaffected.
- **JSON convention**: when a required file is named `{skill}.json`, the gate requires the packager handshake — `source == skill`, non-empty `schema_version`, and non-empty `generated_at` (rejects hand-written shells that omit packager fields). Known article-body skill names also require non-empty `title`/`content_html` (`content-article-review` additionally needs `review.status`); research needs a `sources` list; cover needs `title` + `cover` object. Other basenames only need to exist (and valid JSON object if `.json`).
