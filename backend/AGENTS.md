...
- **Per-tool enablement**: `mcpServers.<server>.tools.<original_tool_name>.enabled` defaults to `true`. `get_mcp_tools()` resolves the original MCP name while the producing server is still known and drops a disabled tool before routing metadata, stdio wrapping, cache exposure, deferred-tool assembly, or agent binding. Configure this with the server's original name, not the default `<server_name>_` bound name.
