...
13. **ToolErrorHandlingMiddleware** - Receives `AppConfig`, converts tool exceptions into error `ToolMessage`s so the run can continue instead of aborting, stamps every result with `deerflow_tool_meta` (status / error_type / recoverable_by_model / recommended_next_action / source) via `tool_result_meta.normalize_tool_result`, stamps structured metadata for task exception wrappers, and stamps skill-read metadata for downstream durable-context capture. Task tool result text is generated from the same status/result/error inputs as the structured metadata so callers do not hand-write a second protocol string.
    - **ToolStreamingMiddleware** - *(optional, if `tool_streaming.enabled`, default off — #4150)* Appended immediately after ToolErrorHandlingMiddleware, i.e. **inner** of it, so raw tool exceptions reach it first. Emits `tool_output_chunk` custom stream events (start / error / final lifecycle); built-in tools currently emit lifecycle status only, while the protocol permits future tools to add intermediate chunks via `langgraph.config.get_stream_writer()`. It re-raises exceptions so ToolErrorHandlingMiddleware converts them to error ToolMessages, re-raises `GraphBubbleUp` control-flow signals without an error chunk, and silently degrades to a pass-through when the custom stream writer is unavailable. The final chunk carries only a short first-line preview (completion signal) — the full result arrives via the canonical ToolMessage. Ordering is pinned by `test_tool_streaming_is_inner_of_tool_error_handling`.
