...
- Scheduled-task IM push (#4254): when `channel_connections.enabled` and a channel service are running, terminal scheduled runs enqueue `notification_deliveries` (`0016`/`0017`); `NotificationDeliveryWorker` pushes via `Channel.send_notification` (WeCom today). Manual/interrupt silent — see `app/channels/AGENTS.md` and `persistence/migrations/AGENTS.md`.
