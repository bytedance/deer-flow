...
- The `RunLauncher` port admits exactly three escapes, and the line between two of them is the #4452 duplicate-execution guard. `ThreadBusyError` is an overlap; `LaunchFailedError` **releases the task's single active slot**, so `GatewayRunLauncher` may raise it only when no run can possibly have started (a 4xx — rejected before the launch path did anything); everything else — a 5xx, an arbitrary exception, or a reply whose `run_id`/`thread_id` will not decode — raises `LaunchIndeterminateError`, which keeps the slot held with `run_id=None` and leaves reconciliation to settle what happened. Reporting "may have started" as a failure is exactly what lets the next dispatch launch a duplicate; the split is pinned by `tests/test_schedule_run_launcher.py`.
- `scheduled_tasks.version` is an optimistic token owned by the storage write path (migration `0011`). Every committed write increments it — `save`'s compare-and-set, `record_launch`, `record_completion`, `claim_due`, `cancel_stuck_once_tasks` — while the aggregate's own transitions leave it alone, because it identifies the snapshot the aggregate was read from. HTTP write use cases are read-modify-write over the whole aggregate, and the dispatch/completion paths write the same row concurrently, so committing a stale snapshot would restore a pre-launch `next_run_at` / `run_count` / `last_run_id` and re-arm an occurrence that already ran. `SqlScheduledTaskRepository.save` is therefore a **conditional UPDATE** (`WHERE version = ?`), not read-check-write: the latter reads and compares in two statements, so two savers can both observe the same version and both commit. A refused write raises `ConcurrentUpdateError`; `ScheduleService` replays the intent against a fresh read for `_SAVE_ATTEMPTS` rounds before letting it surface, and the router maps it to a retryable 409. Contract cases run against both the in-memory double and real sqlite (`TestOptimisticConcurrency`).
...
- `migrations/versions/0011_scheduled_task_version.py` — adds `scheduled_tasks.version`, the optimistic-concurrency token the schedule aggregate carries; every committed write increments it and `save` compares against it, so a read-modify-write racing a dispatch/completion is refused instead of rolling back the fields those writes own. Existing rows start at 0, which is also the aggregate's default, so a task read before the migration and saved after it still commits. Chains after `0010_run_cancel_request`
