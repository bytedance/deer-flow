...
- Scheduled-task dispatch enforces one active occurrence per task via `uq_scheduled_task_run_active` (`task_id WHERE status IN ('queued','launching','running')`). `queued` survives restart, `launching` holds the short launch lease, and `running` is tied to the durable run. Stable admission IDs make recovered launches idempotent; reused-thread `ConflictError`s requeue, while other launch errors become terminal `failed`. Waiting rows do not consume `max_concurrent_runs`; atomic queue claim enforces the budget. Repeated triggers coalesce, same-thread FIFO treats older active rows as blockers, and queue admission/PATCH/resume/pause/delete serialize on the parent before touching the occurrence. Pause/delete interrupt `queued` rows and reject `launching`/`running`; PATCH/resume reject every active state. Manual triggers may run while the schedule is paused. Recovery locks task/run pairs in deterministic order, reconstructs launch bookkeeping before releasing claims, writes occurrence+parent completion in one parent-first transaction, and queue timeout advances scheduled work so it cannot immediately requeue. Repository boundaries coerce serialized task timestamps before SQL `DateTime` binding.
- Once-task recovery reads the deterministic latest run row (`scheduled_for`, then `created_at`, then `id`) and finalises the parent from terminal outcomes: success→completed, failed→failed, interrupted/skipped→cancelled; active rows stay running for a later pass.
