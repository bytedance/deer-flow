...
- **Per-agent override** (issue #3626): a custom agent can opt out of memory by hand-authoring a `memory:` block in its `config.yaml` (`memory: {enabled: false}`) — useful for stateless "execution machine" agents that should not accumulate their own memory even when the deployment enables memory globally. `AgentConfig.memory` (`config/agents_config.py::AgentMemoryConfig`) defaults to `None`, so every existing agent inherits the global config unchanged. `agents/lead_agent/agent.py::apply_agent_memory_override` folds the opt-out into the effective `AppConfig` once, right after `load_agent_config`, so all four memory gates read the already-resolved `app_config.memory` with no per-gate flag: MemoryMiddleware capture, the `memory_*` tools, the `<memory>` injection in `DynamicContextMiddleware`, and the summarization memory-flush hook. The override is narrow by design — an agent may only disable memory, never re-enable it when the operator disabled memory globally (the global switch stays the master kill switch). Like the `github:` block it is a hand-authored field: it is intentionally absent from `MANAGED_AGENT_CONFIG_FIELDS`, so `preserve_non_managed_fields` carries it through the `update_agent` tool / `PATCH /api/agents/{name}` rewrites instead of dropping it. Tests: `tests/test_agent_memory_toggle.py`.
