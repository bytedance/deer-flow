...
- Every Gateway SSE route reuses `app.gateway.services.SSE_RESPONSE_HEADERS`. Keep both `Cache-Control: no-cache, no-transform` (prevents intermediary re-encoding or compression) and `X-Accel-Buffering: no` (disables nginx response buffering) on new streaming endpoints.
