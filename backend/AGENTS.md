...
**Runtime**:
- `make dev`, Docker dev, and production all run the agent runtime in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/deerflow/runtime/`). Nginx exposes that runtime at `/api/langgraph/*` and rewrites it to Gateway's native `/api/*` routers. Terminal runs evict their in-memory registry record event-driven (`RunManager.evict_finished_run()`), bounding the process-local registry (each record retains the run's full `kwargs["input"]`); the full eviction contract — completion-snapshot durability, repair, lease fencing, retry lifecycle — lives in `packages/harness/deerflow/runtime/AGENTS.md`.
