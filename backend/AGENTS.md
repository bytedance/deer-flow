...
- WeCom owns the SDK `connect()` task for the full channel lifecycle. `stop()` cancels that task, calls the SDK `disconnect()`, and awaits cancellation cleanup before clearing `_ws_task` / `_ws_client` or returning; otherwise an immediate restart can overlap a new connection with an untracked task from the previous lifecycle. `CancelledError` is normal shutdown, while real connection failures remain reported by `_on_ws_task_done`.
