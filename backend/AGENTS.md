...
- **Installation**: `POST /api/skills/install` extracts .skill ZIP archive to custom/ directory `POST /api/skills/install/upload` accepts a direct multipart offline package upload. Both routes are admin-only, run the same archive validation and security scans, install into the user-scoped custom directory, and refresh that user's skill prompt cache.
