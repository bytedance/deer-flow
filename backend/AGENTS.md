...
- Ordinary Gateway runs resolve their default LangGraph super-step budget from top-level `recursion_limit` (default 100) on every run. A valid request value takes precedence; invalid values fall back to the configured default, and `max_recursion_limit` caps both sources. Because `build_run_config()` reads `get_app_config()` per run, edits apply without a Gateway restart.
