...
- Community HTTP clients must set an explicit transport-level deadline on
  every external request. InfoQuest uses a shared 30-second request deadline
  for reader, web-search, and image-search calls; its `fetch_timeout` payload
  option configures the remote crawl and does not replace the local socket
  deadline.
