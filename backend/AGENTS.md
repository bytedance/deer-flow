...
14. **DynamicContextMiddleware** - Injects the current date (and optionally memory) as a `<system-reminder>` into the first HumanMessage, keeping the base system prompt fully static for prefix-cache reuse. Built-in subagents use the same middleware in date-only mode (`include_memory=False`) via `build_subagent_runtime_middlewares`, so delegated tasks receive the same framework-owned current-date anchor without loading per-user memory
