...
- **Executable classification parity**: Archive installs and agent-managed support-file writes share `skills/code_files.py`. Files under the top-level `scripts/` directory, known code suffixes anywhere in the tree, and extensionless files beginning with a shebang all receive the executable LLM policy; unrelated paths that merely contain a nested `scripts` component do not.
