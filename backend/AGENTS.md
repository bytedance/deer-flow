...
- When auto-convert writes a companion `.md`, Gateway and `DeerFlowClient` persist `original → companion` (plus the companion's convert-time size/mtime fingerprint) in a hidden `.deer-flow-companions.json` sidecar next to the uploads (not swept on startup). `list_uploaded_files` and outline resolution prefer that mapping so collision-renamed companions (`a.pdf` → `a_1.md`) survive summarization; delete uses the same map instead of always unlinking `<stem>.md`. An entry whose companion file no longer matches its fingerprint (deleted or replaced outside the delete API) is treated as stale: it is not attached to the original and is not removed with it. A replacement reusing a collision-renamed companion name (`a_1.md`) is listed as a user file; a same-stem replacement (`report.md` beside `report.pdf`) remains hidden by the legacy same-stem heuristic. Version-1 rows without a fingerprint verify by existence only.
