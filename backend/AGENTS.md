...
- Scheduled occurrences use a parent-locked `occurrence_seq` for recovery and parent-write eligibility; older launches still reconcile their own history but cannot replace newer parent results. `launch_accounted` and `run_count` commit together; completion writes occurrence, accounting, and eligible parent outcome atomically. These fields stay internal. Legacy NULLs retain best-effort timestamp/last-run inference; no historical backfill. Upgrade all Gateway writers together (see persistence/migrations/AGENTS.md).
