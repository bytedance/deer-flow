...
- The background scheduler is single-instance. Gateway startup rejects `scheduler.enabled=true` with `GATEWAY_WORKERS > 1`; multi-pod deployments must enable it on exactly one Gateway pod.
