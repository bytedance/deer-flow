...
- Query-aware recall uses `MemoryManager.aget_context` on the Gateway path.
  Backends opt in with `supports_query_aware_context = True`; capability-off
  backends never receive the optional `query` keyword.
...
- The shared `agents/memory/context.py` loader owns recall gating and `<memory>`
  wrapping for both prompt/session and middleware/turn consumers. Prompt recall
  rethrows `MemoryManagerError` only when backend config declares
...
14. **DynamicContextMiddleware** - Injects the current date and optional baseline memory into the first turn, and can add query-aware memory as a request-only hidden `HumanMessage` immediately before the latest real user message. Turn recall is cached in redacted run context once per user turn, so tool-loop model calls reuse identical text without writing it to graph state, checkpoints, history, or memory capture input
...
- The optional `openviking` backend under
  ...
  DeerFlow owns recall/capture timing, the baseline injection query, optional
  latest-user-query recall, and the full-transcript suffix cursor.
  `langchain-openviking` owns SDK transport,
...
- `enabled` - Master switch for the memory mechanism
- `injection_enabled` - Master switch for automatic injection
- `session_injection_enabled` - Persist the baseline session snapshot (default: `true`)
- `turn_injection_enabled` - Recall against the latest real user query when supported (default: `false`)
...
- `memory` - Memory system (enabled, mode, injection_enabled, session_injection_enabled, turn_injection_enabled, shutdown_flush_timeout_seconds, manager_class, backend_config)
