...
- Scheduled occurrences use a parent-locked `occurrence_seq` for recovery and parent-write eligibility; older launches still reconcile their own history but cannot replace newer parent results. `launch_accounted` and `run_count` commit together; completion writes occurrence, accounting, and eligible parent outcome atomically. These fields stay internal. Both once-task recovery paths lock the parent row, defer while any occurrence row is active (sequenced or not: `uq_scheduled_task_run_active` makes a live row the newest admission), and otherwise project only from the highest sequence (`can_project`); unsequenced rows (legacy history or a pre-upgrade writer) are projected by their own writer, and a task with only unsequenced history keeps best-effort timestamp/last-run inference; no historical backfill.
