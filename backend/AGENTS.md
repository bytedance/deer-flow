...
**Storage backends**: `memory.storage_class` reflection-loads any `MemoryStorage` subclass (`get_memory_storage()`; unimportable/misconfigured classes log an error and fall back to `FileMemoryStorage`). Built-in: `FileMemoryStorage` (default, per-user `memory.json`). Community prototype: `deerflow.community.kurrentdb.memory_storage.KurrentdbMemoryStorage` — event-sourced storage appending `MemoryUpdated` events to per-user KurrentDB streams (`deerflow.memory-{user_id}[.agent.{agent}]`), cache-first reads, sync `kurrentdbclient` (optional `deerflow-harness[kurrentdb]` extra + `KURRENTDB_CONNECTION_STRING`), dev instance via `docker/docker-compose.kurrentdb.yaml`. On each successful `save()` it also best-effort dual-writes NEW facts as canonical [kurrent-agents](https://github.com/kurrent-io/kurrent-agents) `FactRetained` events to `AgentMemory-deerflow-{user_id}` (via the optional `kurrent-agent-schema` package), so other kurrent-agents-compliant frameworks can read deer-flow's retained facts. See [docs/KURRENTDB_MEMORY.md](docs/KURRENTDB_MEMORY.md).
