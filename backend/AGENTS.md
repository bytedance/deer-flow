...
- `middlewares` - List of `module:Attribute` colon-paths to `AgentMiddleware` subclasses inserted into the lead-agent middleware chain at build time (`agents/factory.py::load_extension_middlewares`, wired as `custom_middlewares` — slot 29 in the Middleware Chain above). Gated by the top-level `enabled` flag (default `true`). Entries are imported and instantiated in-process — operator-trusted code, same trust model as `mcpServers` commands and `mcpInterceptors`; missing modules are skipped at INFO (optional-dependency pattern), other failures at ERROR. `PUT /api/mcp/config` never writes this key (it rewrites only `mcpServers`/`skills` and preserves the rest of the file verbatim). `config.yaml -> extensions` can override these fields after validation; overrides are replace-per-field, not list concatenation.
