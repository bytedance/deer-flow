...
- LocalSandbox keeps host paths behind virtual mount paths during file round-trips. Agent-written content uses forward slashes on Windows to avoid escape corruption, so reverse-output matching must remain separator-agnostic and recognize both that spelling and native backslashes.
