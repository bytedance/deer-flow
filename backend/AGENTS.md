...
- `middlewares` - List of `module:Attribute` colon-paths to `AgentMiddleware` subclasses inserted into the lead-agent middleware chain at build time (`agents/factory.py::load_extension_middlewares`, wired as the **Custom middlewares** slot in the Middleware Chain above — just before the terminal-response/safety/clarification tail). Gated by the top-level `enabled` flag (default `true`). Entries are imported and instantiated in-process — operator-trusted code, same trust model as `mcpServers` commands and `mcpInterceptors`. Loading fails closed: only an absent declared module (the underlying `ModuleNotFoundError` names the declared module or one of its parent packages) is skipped at INFO as an optional dependency; malformed paths, missing attributes, transitive import failures, non-`AgentMiddleware` classes, and constructor failures abort agent construction with an actionable error, so a typo cannot silently disable a configured guard middleware. `PUT /api/mcp/config` never writes this key (it rewrites only `mcpServers`/`skills` and preserves the rest of the file verbatim). `config.yaml -> extensions` can override these fields after validation; overrides are replace-per-field, not list concatenation.
