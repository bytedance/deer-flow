...
- **Schema normalization**: DeerMem reads/imports canonicalize legacy sections and recoverable fact metadata; facts without usable content are dropped during normalization
...
- DeerMem canonicalizes the memory schema at load/import/save boundaries so older additive schemas remain readable and persisted files converge on the current structure.
