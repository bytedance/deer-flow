...
**Persistent CodingTask binding**: `task()` accepts an optional `coding_task_id`. When present, it resolves the current user/thread TaskGraph, claims that task with the selected `subagent_type` before dispatch, and completes it only after a successful terminal subagent result. Failed, cancelled, timed-out, disappeared, launch-error, parent-cancelled, and unexpected-exception paths call the same CodingTask failure transition and persist a reason; `TaskGraph.recover()` explicitly resets a failed task to pending and clears its owner/reason before a new claim. Calls without `coding_task_id` preserve the original delegation behavior.
**Coding skill wiring**: `skills/public/multi-agent-coding/SKILL.md` allows only `submit_task_plan` and `task`. After its input gate, it persists the stable `coding-analysis -> coding-implementation -> coding-review` stage DAG before delegation and supplies the matching `coding_task_id` on every subagent call.
