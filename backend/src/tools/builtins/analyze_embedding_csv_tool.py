...
@tool("analyze_embedding_csv", parse_docstring=True)
def analyze_embedding_csv_tool(
    ...
    """Analyze 2D embedding CSV(s) (t-SNE/UMAP) to produce auditable separation & batch-effect metrics + reproduction script.

    Outputs are written under `/mnt/user-data/outputs/scientific-vision/raw-data/embedding/` and added to `artifacts`.

    Args:
        csv_paths: List of virtual paths to CSV files under `/mnt/user-data/...` (typically `/mnt/user-data/uploads/*.csv`).
        x_col: Optional x coordinate column name (auto-detected if not provided).
        y_col: Optional y coordinate column name (auto-detected if not provided).
        cluster_col: Optional cluster/label column name (auto-detected if not provided).
        batch_col: Optional batch/sample column name (auto-detected if not provided).
        knn_k: k for kNN batch mixing score (default 30).
    """
    ...
    try:
        payload, artifacts = analyze_embedding_csv_files(
        ...
        logger.exception("analyze_embedding_csv failed (thread_id=%s): %s", thread_id, exc)
        return Command(update={"messages": [ToolMessage(f"Error: analyze_embedding_csv failed: {exc}", tool_call_id=tool_call_id)]})

    msg = f"analyze_embedding_csv completed: inputs={len(payload.get('inputs') or [])}, artifacts={len(artifacts)}. signature={payload.get('analysis_signature')}"
