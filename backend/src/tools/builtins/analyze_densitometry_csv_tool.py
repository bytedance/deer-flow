...
@tool("analyze_densitometry_csv", parse_docstring=True)
def analyze_densitometry_csv_tool(
    ...
    """Analyze densitometry/quantification CSV(s) (e.g., Western blot band intensity tables) into auditable aggregates + reproduction script.

    Outputs are written under `/mnt/user-data/outputs/scientific-vision/raw-data/densitometry/` and added to `artifacts`.

    Args:
        csv_paths: List of virtual paths to CSV files under `/mnt/user-data/...` (typically `/mnt/user-data/uploads/*.csv`).
        sample_col: Optional sample/lane identifier column (auto-detected if not provided).
        target_col: Optional protein/target/band identifier column (auto-detected if not provided).
        value_col: Optional numeric intensity/density column (auto-detected if not provided).
        control_target: Optional target name for within-sample normalization (e.g. 'GAPDH', 'β-actin').
    """
    ...
    try:
        ...
        logger.exception("analyze_densitometry_csv failed (thread_id=%s): %s", thread_id, exc)
        return Command(update={"messages": [ToolMessage(f"Error: analyze_densitometry_csv failed: {exc}", tool_call_id=tool_call_id)]})

    msg = f"analyze_densitometry_csv completed: inputs={len(payload.get('inputs') or [])}, artifacts={len(artifacts)}. signature={payload.get('analysis_signature')}"
