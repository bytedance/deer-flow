...
@router.patch(
    ...
    description=(
        "Update memory configuration at runtime. Changes take effect immediately. "
        "`injection_enabled` is also persisted to the memory storage file so the LangGraph "
        "process can read it cross-process, while other settings are only kept in-process "
