...
- **Path round-trips**: Agent-written files are translated back to virtual paths when read, including mixed `/` and `\` spellings on Windows, so host paths stay hidden from the agent
