...
- **文档：** 新增中英文扩展开发手册（`harness/extensions/`），覆盖
  `deerflow-extension-api` 0.2.3 契约：何时编写扩展、快速上手、运行时模型、中间件
  放置位置、生命周期与观察者、服务与路由、运行证据读取器、实验性的全栈插件、扩展
  运维、按错误信息排查，以及列出全部公开名称和契约版本历史的参考章节。同时修正
  `AGENTS.md` 中对贡献类型的过时描述，以及 `docs/full-stack-plugins.md` 中插件
  `mount` 返回值的描述。
