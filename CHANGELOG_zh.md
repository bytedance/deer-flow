...
- **提示词注入：** 修复两处输入净化绕过。`hide_from_ui` 与人类消息上的
  ...
  过净化；现在这两者都会从不可信的 run 输入与线程状态写入中剥离，调用方再也无法把
  原样的 `<system-reminder>` 放到 user-input 边界标记之外——而主智能体 prompt 正是
  把边界外的内容声明为可信的框架数据。HumanInputCard 的回复凭其校验过的
  `human_input_response` 保留 `hide_from_ui` 并照常被净化，受信任的内部启动路径不受
  影响。净化范围也从"仅最新一轮"扩大到*每一条*真实用户消息：该变换只作用于单次请
