...
def check_x_search(config_path: Path) -> CheckResult:
    return check_web_tool(config_path, tool_name="x_search", label="X search configured", optional=True)


def check_web_tool(config_path: Path, *, tool_name: str, label: str, optional: bool = False) -> CheckResult:
    ...
    try:
        ...
        if not tool_entries:
            if optional:
                return CheckResult(label, "skip", f"optional {tool_name} tool not configured")
            return CheckResult(
                label,
                "warn",
